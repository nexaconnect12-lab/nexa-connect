...
public sealed class PostgresReportingReadRepository(NpgsqlDataSource dataSource) : IReportingReadRepository
{
    ...
    {
        const string sql = "WITH currencies AS (SELECT currency FROM sales_facts WHERE organization_id=$1 AND branch_id=$2 AND ordered_at_utc >= $3 AND ordered_at_utc < $4 UNION SELECT currency FROM payment_facts WHERE organization_id=$1 AND branch_id=$2 AND paid_at_utc >= $3 AND paid_at_utc < $4) SELECT count(*) FILTER (WHERE order_status='completed')::int,COALESCE(sum(total_amount) FILTER (WHERE order_status='completed'),0),COALESCE((SELECT sum(paid_amount-refunded_amount) FROM payment_facts p WHERE p.organization_id=$1 AND p.branch_id=$2 AND p.paid_at_utc >= $3 AND p.paid_at_utc < $4),0),COALESCE((SELECT sum(refunded_amount) FROM payment_facts p WHERE p.organization_id=$1 AND p.branch_id=$2 AND p.paid_at_utc >= $3 AND p.paid_at_utc < $4),0),(SELECT min(currency) FROM currencies),(SELECT count(*) FROM currencies),(SELECT max(updated_at_utc) FROM projection_checkpoints) FROM sales_facts s WHERE s.organization_id=$1 AND s.branch_id=$2 AND s.ordered_at_utc >= $3 AND s.ordered_at_utc < $4;";
        await using var connection = await dataSource.OpenConnectionAsync(cancellationToken);
        await using var command = Command(sql, connection, range);
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        await reader.ReadAsync(cancellationToken);
        if (reader.GetInt64(5) > 1) throw new MixedReportingCurrencyException();
        return new(reader.GetInt32(0), reader.GetDecimal(1), reader.GetDecimal(2), reader.GetDecimal(3), reader.IsDBNull(4) ? null : reader.GetString(4), reader.IsDBNull(6) ? null : reader.GetFieldValue<DateTimeOffset>(6));
    }
    ...
    public async Task<SalesReport> SalesAsync(ReportingRange range, CancellationToken cancellationToken)
    {
        ...
        await using var aggregate = Command("SELECT COALESCE(sum(total_amount) FILTER (WHERE order_status='completed'),0),count(DISTINCT currency),min(currency) FROM sales_facts WHERE organization_id=$1 AND branch_id=$2 AND ordered_at_utc >= $3 AND ordered_at_utc < $4;", connection, range);
        ...
        if (aggregateReader.GetInt64(1) > 1) throw new MixedReportingCurrencyException();
        string? currency = aggregateReader.IsDBNull(2) ? null : aggregateReader.GetString(2);
        ...
        return new(range, items, totalSales, currency, fresh);
    }
