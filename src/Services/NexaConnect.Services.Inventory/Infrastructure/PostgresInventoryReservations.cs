...
public sealed class PostgresInventoryReservations(NpgsqlDataSource dataSource) : IInventoryReservations
{
    ...
    public StockReservation Reserve(Guid organizationId, ReserveStock command,InventoryMutationContext? context=null)
    {
        ...
        using(var idempotencyLock=new NpgsqlCommand("SELECT pg_advisory_xact_lock(hashtextextended(@key,0))",connection,transaction)){idempotencyLock.Parameters.AddWithValue("key",$"{organizationId:D}:{command.OrderId:D}");idempotencyLock.ExecuteNonQuery();}
        using(var existing=new NpgsqlCommand("SELECT reservation_id FROM inventory_reservation_lines WHERE organization_id=@organization AND order_id=@order AND released_at_utc IS NULL LIMIT 1",connection,transaction)){existing.Parameters.AddWithValue("organization",organizationId);existing.Parameters.AddWithValue("order",command.OrderId);if(existing.ExecuteScalar() is Guid existingId){transaction.Rollback();return new StockReservation(existingId,command.OrderId,command.BranchId,command.Lines);}}
    ...
    {
        context=Require(context);using var connection=dataSource.OpenConnection();using var transaction=connection.BeginTransaction();using var command = new NpgsqlCommand("WITH released AS (UPDATE inventory_reservation_lines SET released_at_utc=now() WHERE organization_id=@organization AND order_id=@order AND released_at_utc IS NULL RETURNING organization_id,branch_id,product_id,quantity), restored AS (UPDATE inventory_stock s SET available_quantity=s.available_quantity+r.quantity FROM released r WHERE s.organization_id=r.organization_id AND s.branch_id=r.branch_id AND s.product_id=r.product_id RETURNING 1) SELECT count(*) FROM released",connection,transaction);
        command.Parameters.AddWithValue("organization", organizationId); command.Parameters.AddWithValue("order", orderId); long affected=(long)(command.ExecuteScalar()??0L);if(affected>0){DateTimeOffset now=DateTimeOffset.UtcNow;var evt=new InventoryReservationReleasedV1(Guid.NewGuid(),context.CorrelationId,now,organizationId,orderId);Audit(connection,transaction,organizationId,Guid.Empty,orderId,"inventory.reservation.released",context,now);Enqueue(connection,transaction,evt.EventId,"inventory.reservation-released.v1",orderId,evt,context,now);}transaction.Commit();
    }
