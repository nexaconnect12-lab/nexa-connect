...
public sealed record InventoryMutationContext(string ActorSubjectId, Guid CorrelationId);
...
public interface IInventoryReservations
{
    ...
    StockItem SetStock(Guid organizationId, Guid branchId, Guid productId, decimal quantity, InventoryMutationContext? context = null) => SetStock(branchId, productId, quantity);
    StockReservation Reserve(ReserveStock command);
    StockReservation Reserve(Guid organizationId, ReserveStock command, InventoryMutationContext? context = null) => Reserve(command);
    void Release(Guid orderId) { }
    void Release(Guid organizationId, Guid orderId, InventoryMutationContext? context = null) => Release(orderId);
}
