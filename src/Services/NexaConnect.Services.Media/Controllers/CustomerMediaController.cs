...
public sealed class CustomerMediaController(MediaAssetQueries queries,MediaManagement management,IMediaCustomerAuthorizer authorizer,ILogger<CustomerMediaController> logger):ControllerBase
{
 ...
 [HttpPost("uploads")]public async Task<IActionResult>Start(Guid organizationId,StartMediaUploadCommand request,CancellationToken c){if(!await Granted(organizationId,ProductPermissions.MediaAssetManage,c))return Forbid();try{return StatusCode(201,await management.StartAsync(organizationId,request,Actor()!,c));}catch(ArgumentException e){return BadRequest(new ProblemDetails{Title=e.Message,Status=400});}}
