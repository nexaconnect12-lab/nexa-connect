...
builder.Services.AddSingleton<IMediaContentSafety,ClamAvMediaContentSafety>();
...
if(!builder.Environment.IsDevelopment()&&!builder.Environment.IsEnvironment("Test")&&!builder.Configuration.GetValue<bool>("MediaSafety:MalwareScanEnabled"))throw new InvalidOperationException("MediaSafety:MalwareScanEnabled is required outside Development and Test.");var app=builder.Build();await using(var schema=app.Services.GetRequiredService<NpgsqlDataSource>().CreateCommand("SELECT to_regclass('public.media_processing_jobs') IS NOT NULL")){if(await schema.ExecuteScalarAsync() is not true)throw new InvalidOperationException("Media migration 4 is required before service startup.");}app.UseNexaConnectRequestLogging();if(app.Environment.IsDevelopment())app.MapOpenApi();app.UseHttpsRedirection();app.UseAuthentication();app.UseAuthorization();app.MapControllers();app.Run();
