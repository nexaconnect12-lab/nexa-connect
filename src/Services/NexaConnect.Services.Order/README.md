...
The workflow publishes versioned contracts from `NexaConnect.Contracts.IntegrationEvents` after each accepted step. PostgreSQL mode persists the aggregate, immutable line snapshots, idempotency record, and integration event in one transaction; the shared outbox dispatcher delivers events to RabbitMQ. The public endpoint is `POST /api/order/v1/workflows/place` and requires an idempotency key. Outbound adapters attach a configured development token or obtain a short-lived Keycloak client-credentials token. A provider timeout or unavailable response leaves the order `payment_pending` and retains Inventory/Kitchen work; `PaymentReconciliationApplicationService` later marks it paid or performs idempotent compensation after an authoritative failed result. When `PaymentReconciliationConsumer:Enabled=true`, the durable inbox consumer subscribes to `payment.authorization-reconciled.v1` and invokes that handler exactly once per event.
