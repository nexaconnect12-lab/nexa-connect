...
For the full migration lifecycle, also set `NEXACONNECT_POS_CLEAN_INSTALL_ACCEPTANCE=1` and `NEXACONNECT_POSTGRES_ADMIN_INTEGRATION_DB` to a disposable Development/Test administrator allowed to create and drop databases, then filter `PosMigrationRunnerAcceptanceTests`. It manages only generated names matching `nexaconnect_pos_clean_it_<guid>`, runs 0→4→3→4, and force-drops that validated database. This lifecycle passed in the guarded live matrix on 2026-09-03 UTC. Never use production credentials.
