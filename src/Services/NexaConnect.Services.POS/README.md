...
The [joined portal acceptance](../../../docs/Deployment/Cash-Close-Portal-Acceptance.md) enables actual publication/outbox workers against disposable infrastructure. Repository-produced close, approval and late-settlement changes reach the browser through Reporting. The POS workload token explicitly carries the API audience for Restaurant hierarchy calls; existing realms require a reviewed mapper update or disposable re-import.
