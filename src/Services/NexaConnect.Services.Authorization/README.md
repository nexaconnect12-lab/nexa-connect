...
`POST /api/authorization/v1/role-assignments` accepts legacy `system-admin` and current `platform-owner`/`platform-admin` control-plane roles. Platform Admin BFF exposes a same-body proxy while Authorization remains the persistence owner.
