...
Operational telemetry uses `NexaConnect.Observability` with service name `nexaconnect-platform-directory`: structured JSON remains on stdout and OTLP export is optional through `Observability__OtlpEnabled` and `Observability__OtlpEndpoint`. Request logs include method, path, status, duration, trace ID, and a validated `X-Correlation-ID`; they exclude bodies, query strings, credentials, cookies, and arbitrary headers. Organization access denial events distinguish a missing stable subject from inactive membership/application access without recording the subject value. Follow the request's `CorrelationId` across Catalog and Platform Directory when diagnosing a POS menu `403`. These logs do not replace the append-only support-elevation audit history.
