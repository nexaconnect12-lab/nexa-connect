...
public sealed class OmiseWebhookWorker(IServiceScopeFactory scopes, IOptions<OmiseWebhookOptions> options,
    IOmiseWebhookProcessingBoundary boundary, ILogger<OmiseWebhookWorker> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                OmiseWebhookProcessResult result;
                ...
                try
                {
                    ...
                    result = await scope.ServiceProvider.GetRequiredService<OmiseWebhookProcessor>().ProcessAsync(claim, stoppingToken);
                }
                catch (Exception exception) when (exception is not OperationCanceledException || !stoppingToken.IsCancellationRequested)
                { result = new("retry", false); logger.LogWarning("Omise webhook processing failed; durable delivery remains retryable"); }
                string outcome = result.Outcome;
                await boundary.AfterProcessingAsync(outcome, result.FinancialTransitionCommitted, stoppingToken);
