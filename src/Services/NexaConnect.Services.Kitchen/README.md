...
Service name is `nexaconnect-kitchen`. All five coordinated Kitchen/Reporting acceptances passed locally against PostgreSQL 17 and RabbitMQ. The shared outbox transport replaces failed owned connections, although the Kitchen-specific broker acceptance does not exercise that path. Item-level workflows, adjustments, offline/dedicated KDS, and canonical station IDs remain planned; online ticket queues are implemented below.
...
## Online operator queue

`GET /api/kitchen/v1/branches/{branchId}/tickets` lists active tickets with optional station filter and keyset cursor (default 50, maximum 100). `KitchenQueueService` owns operator authorization and orchestration; the persistence adapters retain version/lifecycle checks under write locks. In-memory cancellation now prevalidates every station ticket, matching PostgreSQL all-or-none cancellation. No schema or event version changes are required. Operator denials return 403; missing/cross-branch tickets after authorization return 404.

The Customer Portal calls through its CSRF-protected BFF and refreshes after conflicts or uncertain outcomes without replay. Preparation is independent of payment; completed tickets block compensation and require investigation. See [API, deployment, diagnostics and acceptance](../../../docs/API/Kitchen-Queue.md). Query `{service_name="nexaconnect-kitchen"} |= "Kitchen operator"` or `|= "Kitchen compensation blocked"` with the validated correlation ID. Live queue acceptance remains separate from the earlier ticket-lifecycle acceptance.
