...
type MediaAsset = { id:string; originalFileName:string; contentType:string; sizeBytes:number; processingStatus:string; uploadedAtUtc:string; ownerService:string };
...
function MediaPanel(){const state=useRequest<MediaAsset[]>("/bff/customer/media");return <><Typography.Title level={2}>Media management</Typography.Title><Alert type="info" message="Media metadata is owned by the dedicated Media service. Upload and object-storage workflows remain staged."/><Table style={{marginTop:16}} rowKey="id" loading={!state.data&&!state.error} dataSource={state.data} columns={[{title:"File",dataIndex:"originalFileName"},{title:"Type",dataIndex:"contentType"},{title:"Owner",dataIndex:"ownerService"},{title:"Size (bytes)",dataIndex:"sizeBytes"},{title:"Status",dataIndex:"processingStatus",render:value=><Tag>{value}</Tag>},{title:"Uploaded",dataIndex:"uploadedAtUtc"}]}/>{state.error&&<Alert type="error" message={state.error}/>}</>}
function FeaturePanel({
  section,
}: {
  section: Exclude<
    PortalSection,
    "profile" | "products" | "users" | "configuration" | "branches" | "dashboards" | "reports" | "media"
  >;
}) {
