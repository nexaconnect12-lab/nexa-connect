...
function MediaPanel(){const state=useRequest<MediaAsset[]>("/bff/customer/media"),[file,setFile]=useState<File>(),[ownerId,setOwnerId]=useState(""),[message,setMessage]=useState<string>();const upload=async()=>{if(!file||!ownerId)return;try{const hash=Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",await file.arrayBuffer()))).map(v=>v.toString(16).padStart(2,"0")).join("");const session=await api.request<{asset:MediaAsset;uploadUrl:string}>("/bff/customer/media/uploads",{method:"POST",body:{ownerService:"catalog",ownerType:"product",ownerId,fileName:file.name,contentType:file.type,sizeBytes:file.size,checksumSha256:hash}});const put=await fetch(session.uploadUrl,{method:"PUT",headers:{"Content-Type":file.type,"x-amz-meta-sha256":hash},body:file});if(!put.ok)throw new Error("Object upload failed.");await api.request(`/bff/customer/media/${session.asset.id}/complete`,{method:"POST",body:{expectedVersion:session.asset.concurrencyVersion}});setMessage("Upload completed.");state.reload()}catch(e){setMessage(normalizeError(e).message)}};const download=async(asset:MediaAsset)=>{const value=await api.request<{downloadUrl:string}>(`/bff/customer/media/${asset.id}/download`);location.assign(value.downloadUrl)};const remove=async(asset:MediaAsset)=>{await api.request(`/bff/customer/media/${asset.id}?expectedVersion=${asset.concurrencyVersion}`,{method:"DELETE"});state.reload()};return <><Typography.Title level={2}>Media management</Typography.Title><Card title="Upload product image"><Space direction="vertical"><Input placeholder="Catalog product UUID" value={ownerId} onChange={e=>setOwnerId(e.target.value)}/><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>setFile(e.target.files?.[0])}/><Button type="primary" disabled={!file||!ownerId} onClick={upload}>Upload</Button></Space></Card>{message&&<Alert style={{marginTop:16}} message={message}/>}<Table style={{marginTop:16}} rowKey="id" loading={!state.data&&!state.error} dataSource={state.data} columns={[{title:"File",dataIndex:"originalFileName"},{title:"Type",dataIndex:"contentType"},{title:"Owner",dataIndex:"ownerService"},{title:"Size (bytes)",dataIndex:"sizeBytes"},{title:"Status",dataIndex:"processingStatus",render:value=><Tag>{value}</Tag>},{title:"Uploaded",dataIndex:"uploadedAtUtc"},{title:"Actions",render:(_,asset)=><Space><Button disabled={asset.processingStatus!=="ready"} onClick={()=>download(asset)}>Download</Button><Button danger onClick={()=>remove(asset)}>Delete</Button></Space>}]}/>{state.error&&<Alert type="error" message={state.error}/>}</>}
