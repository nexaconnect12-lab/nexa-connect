...
describe("Product Owner Portal boundary", () => {
  it("keeps support away from platform mutations and product administration", () => {
    const c = capabilitiesFor(["platform-support"]);
    expect(c.has("support.request")).toBe(true);
    expect(c.has("organizations.manage")).toBe(false);
    expect(c.has("admin-links.open")).toBe(false);
  });
  ...
  it("allows only controlled same-origin or HTTPS links", () => {
    const links = parseAdminLinks(
      "nexa|Nexa Admin|https://admin.example.test,evil|Bad|javascript:alert(1),local|Local|/admin",
      "https://owner.example.test",
    );
    expect(links.map((x) => x.applicationCode)).toEqual(["nexa", "local"]);
  });
  it("allows platform administrators to provision hierarchy and product roles", () => {
    const c = capabilitiesFor(["platform-admin"]);
    expect(c.has("hierarchy.manage")).toBe(true);
    expect(c.has("product-roles.manage")).toBe(true);
  });
  ...
  it("uses a controlled application catalog with immutable application codes", () => {
    expect(
      applicationOptions(
        "delivery|Delivery,invalid code|Invalid,core|Core,core|Duplicate",
      ),
    ).toEqual([
      { value: "core", label: "Core (core)" },
      { value: "delivery", label: "Delivery (delivery)" },
    ]);
    expect(applicationOptions(undefined)).toEqual([
      { value: "nexa_connect", label: "NexaConnect (nexa_connect)" },
    ]);
  });
  it("presents hierarchy labels while retaining restaurant and branch IDs", () => {
    expect(
      restaurantOptions([
        {
      ...
      { value: "restaurant-1", label: "Main Restaurant (main) · active" },
    ]);
    expect(
      branchOptions([
        {
      ...
      { value: "branch-1", label: "Orchard Branch (orchard) · active" },
    ]);
  });
});
