...
function configured(){
  const values={ENABLED:"1",CONFIRM_DISPOSABLE:"1",RUN_ID:"a".repeat(32),BASE_URL:"https://localhost:51829",OIDC_ISSUER:`http://localhost:18080/realms/nexa-review-it-${"a".repeat(32)}`,RESOLVER_USERNAME:"resolver",RESOLVER_PASSWORD:"synthetic-secret",READER_USERNAME:"reader",READER_PASSWORD:"synthetic-secret-two"};
  for(const [i,key] of ["ORGANIZATION_ID","OTHER_ORGANIZATION_ID","BRANCH_ID","CONCURRENCY_ORDER_ID","RESUME_ORDER_ID","VOID_ORDER_ID","LOST_RESPONSE_ORDER_ID"].entries())values[key]=`${i+1}`.repeat(8)+"-1111-1111-1111-111111111111";
...
test("zero, partial, skipped and retried runs cannot claim complete evidence",()=>{
  const passes=Array.from({length:6},(_,i)=>({title:`scenario-${i}`,status:"passed"}));
  assert.equal(completeEvidence("passed",passes),true);
  for(const results of [[],passes.slice(1),[...passes,{status:"passed"}],[...passes.slice(1),{status:"skipped"}],Array(6).fill(passes[0])])assert.equal(completeEvidence("passed",results),false);
