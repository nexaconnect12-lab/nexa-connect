...
builder.Services.AddHttpClient("Notification",client=>client.BaseAddress=new Uri(builder.Configuration["Services:Notification"]??throw new InvalidOperationException("Services:Notification is required."))).AddNexaConnectCorrelationPropagation();
...
app.MapGet("/bff/customer/notifications/{notificationId:guid}",async(Guid notificationId,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger<Program> logger,CancellationToken c)=>await ProxyCustomerNotification(notificationId,context,clients,cookie,logger,c)).RequireAuthorization("CustomerSession");
...
app.MapPost("/bff/customer/media/uploads",async(HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger<Program> logger,CancellationToken c)=>await ProxyCustomerMedia("uploads",context,clients,cookie,logger,c)).RequireAuthorization("CustomerSession");
app.MapPost("/bff/customer/media/{assetId:guid}/complete",async(Guid assetId,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger<Program> logger,CancellationToken c)=>await ProxyCustomerMedia($"{assetId:D}/complete",context,clients,cookie,logger,c)).RequireAuthorization("CustomerSession");
app.MapGet("/bff/customer/media/{assetId:guid}/download",async(Guid assetId,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger<Program> logger,CancellationToken c)=>await ProxyCustomerMedia($"{assetId:D}/download",context,clients,cookie,logger,c)).RequireAuthorization("CustomerSession");
app.MapGet("/bff/customer/media/{assetId:guid}/variants",async(Guid assetId,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger<Program> logger,CancellationToken c)=>await ProxyCustomerMedia($"{assetId:D}/variants",context,clients,cookie,logger,c)).RequireAuthorization("CustomerSession");
app.MapGet("/bff/customer/media/{assetId:guid}/variants/{name}/download",async(Guid assetId,string name,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger<Program> logger,CancellationToken c)=>await ProxyCustomerMedia($"{assetId:D}/variants/{Uri.EscapeDataString(name)}/download",context,clients,cookie,logger,c)).RequireAuthorization("CustomerSession");
app.MapDelete("/bff/customer/media/{assetId:guid}",async(Guid assetId,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger<Program> logger,CancellationToken c)=>await ProxyCustomerMedia(assetId.ToString("D"),context,clients,cookie,logger,c)).RequireAuthorization("CustomerSession");
...
static async Task<IResult> ProxyCustomerMedia(string suffix,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger logger,CancellationToken cancellationToken)
{
 TenantContext? tenant=ValidTenant(context,cookie);if(tenant is null||tenant.ApplicationCode!="nexa_connect")return Results.Unauthorized();string? token=await GetCustomerAccessTokenAsync(context,cancellationToken);if(string.IsNullOrWhiteSpace(token))return Results.Unauthorized();if(context.Request.ContentLength is > 16_384)return Results.StatusCode(StatusCodes.Status413PayloadTooLarge);string target=$"api/media/v1/customer/organizations/{tenant.OrganizationId:D}/assets/{suffix}"+context.Request.QueryString;using var request=new HttpRequestMessage(new HttpMethod(context.Request.Method),target);request.Headers.Authorization=new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer",token);if(context.Request.Method is "POST"){try{request.Content=await BoundedJsonContent(context.Request,16_384,cancellationToken);}catch(BadHttpRequestException){return Results.StatusCode(StatusCodes.Status413PayloadTooLarge);}}using HttpResponseMessage response=await clients.CreateClient("Media").SendAsync(request,cancellationToken);if(!response.IsSuccessStatusCode)logger.LogWarning("Customer media downstream failure for organization {OrganizationId}, method {Method}, status {StatusCode}",tenant.OrganizationId,context.Request.Method,(int)response.StatusCode);return await ForwardJsonAsync(response,cancellationToken);
}

static async Task<IResult> ProxyCustomerNotification(Guid notificationId,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger logger,CancellationToken cancellationToken)
{
 TenantContext? tenant=ValidTenant(context,cookie);if(tenant is null||tenant.ApplicationCode!="nexa_connect")return Results.Unauthorized();string? token=await GetCustomerAccessTokenAsync(context,cancellationToken);if(string.IsNullOrWhiteSpace(token))return Results.Unauthorized();using var request=new HttpRequestMessage(HttpMethod.Get,$"api/notification/v1/notifications/{notificationId:D}");request.Headers.Authorization=new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer",token);request.Headers.TryAddWithoutValidation("X-Nexa-Organization-Id",tenant.OrganizationId.ToString("D"));request.Headers.TryAddWithoutValidation("X-Nexa-Application-Code",tenant.ApplicationCode);using HttpResponseMessage response=await clients.CreateClient("Notification").SendAsync(request,cancellationToken);if(!response.IsSuccessStatusCode)logger.LogWarning("Customer Notification query failed for organization {OrganizationId}, notification {NotificationId}, status {StatusCode}",tenant.OrganizationId,notificationId,(int)response.StatusCode);return await ForwardJsonAsync(response,cancellationToken);
}
...
static async Task<HttpContent> BoundedJsonContent(HttpRequest request,int maximumBytes,CancellationToken cancellationToken)
{
 byte[] buffer=new byte[4096];using var content=new MemoryStream();while(true){int read=await request.Body.ReadAsync(buffer,cancellationToken);if(read==0)break;if(content.Length+read>maximumBytes)throw new BadHttpRequestException("Request body is too large.",StatusCodes.Status413PayloadTooLarge);await content.WriteAsync(buffer.AsMemory(0,read),cancellationToken);}return new ByteArrayContent(content.ToArray()){Headers={ContentType=System.Net.Http.Headers.MediaTypeHeaderValue.Parse(request.ContentType??"application/json")}};
}
