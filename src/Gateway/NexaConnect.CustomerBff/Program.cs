...
app.MapMethods("/bff/customer/media/{**path}",["GET","POST","DELETE"],async(string? path,HttpContext context,IHttpClientFactory clients,TenantSelectionCookie cookie,ILogger<Program> logger,CancellationToken c)=>{TenantContext? tenant=ValidTenant(context,cookie);if(tenant is null||tenant.ApplicationCode!="nexa_connect")return Results.Unauthorized();string? token=await GetCustomerAccessTokenAsync(context,c);if(string.IsNullOrWhiteSpace(token))return Results.Unauthorized();string target=$"api/media/v1/customer/organizations/{tenant.OrganizationId:D}/assets/{path}"+context.Request.QueryString;using var request=new HttpRequestMessage(new HttpMethod(context.Request.Method),target);request.Headers.Authorization=new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer",token);if(context.Request.ContentLength>0)request.Content=await ReplayableJsonContent(context.Request,c);using var response=await clients.CreateClient("Media").SendAsync(request,c);if(!response.IsSuccessStatusCode)logger.LogWarning("Customer media mutation downstream failure for organization {OrganizationId}, method {Method}, status {StatusCode}",tenant.OrganizationId,context.Request.Method,(int)response.StatusCode);return await ForwardJsonAsync(response,c);}).RequireAuthorization("CustomerSession");
