...
Kitchen queue routes use `/bff/customer/kitchen`: GET `csrf`, `branches/{branchId}/tickets`, and `branches/{branchId}/tickets/{ticketId}`; POST `branches/{branchId}/tickets/{ticketId}/transitions`. The Application service revalidates protected membership and the adapter forwards only server-derived tenant context. POST requires `X-Nexa-CSRF` and the existing shared antiforgery cookie; it accepts only InProgress/Ready/Completed plus a positive expected version. `Services__Kitchen` must point to the actual Kitchen host (Development default `https://localhost:52273/`); the client timeout is 15 seconds. All requests propagate correlation, errors suppress downstream bodies, and responses are no-store. Query `{service_name="nexaconnect-customer-bff"} |= "Kitchen BFF"`. See [the contract and deployment gates](../../../docs/API/Kitchen-Queue.md).

The [joined cash-close portal suite](../../../docs/Deployment/Cash-Close-Portal-Acceptance.md) exercises real OIDC sessions, tenant switching and this BFF against Reporting/POS authorization, including existing-session revocation and unavailable dependencies. It publishes this BFF with the portal assets and uses generated loopback redirects; no new production route is introduced.
