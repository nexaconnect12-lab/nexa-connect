# NexaConnect Observability

Shared operational telemetry for ASP.NET Core services. `AddNexaConnectObservability` configures structured JSON console logs, OpenTelemetry logs/traces/metrics, service/environment resource attributes, and optional OTLP export. `UseNexaConnectRequestLogging` adds safe request completion logs and propagates a validated `X-Correlation-ID`.

The library deliberately does not log request/response bodies, query strings, authorization headers, cookies, tokens, or arbitrary headers. Services must not add secrets, payment data, or unrestricted personal data as log properties.

Configuration:

```json
{
  "Observability": {
    "OtlpEnabled": true,
    "OtlpEndpoint": "http://localhost:4317",
    "ServiceVersion": "optional-release-version"
  }
}
```

Console logging always remains enabled. If OTLP is disabled, no collector is required. If enabled, the endpoint must be an absolute HTTP(S) URI; exporter delivery failures do not stop request handling. Operational logs are not an audit ledger and must not replace service-owned immutable business audit records.

For request-chain clients, append `.AddNexaConnectCorrelationPropagation()` to the `AddHttpClient` registration. The handler first uses the correlation identifier validated by request middleware. A background worker without an HTTP context may wrap its operation in `CorrelationContext.Push` with a service-owned, validated identifier; the handler then forwards that ambient value and restores the prior context when the scope ends. Workers must derive this value from trusted durable state or generate it locally and must not copy an unchecked external header. The handler never forwards arbitrary inbound headers.

CreateClientLoggerFactory supports native client hosts using the same JSON console/OpenTelemetry logging foundation. It requires a service name and accepts optional ObservabilityOptions for OTLP log export. Callers own/dispose the factory and emit only allow-listed safe properties. The POS checkout client uses nexaconnect-pos-client with export disabled by default; it does not register ASP.NET request middleware or automatic HTTP instrumentation.
