...
The **Cash review** tab reads closed sessions for the configured store over a selectable range (up to 31 days), pages older results, displays financial movements and immutable decisions, and lets an authorized resolver mark a nonzero variance investigating or approved with an explicit reason and confirmation. The client sends both financial and review versions. A conflict refreshes authoritative state instead of retrying. An uncertain response retains the same operation identity for an identical retry while the process remains running; after restart, refresh history before acting because this online-only supervisor attempt is not stored in SQLite. A late settlement can return an approved session to review required. Read-only users can inspect history but cannot submit decisions.
...
See [cashier acceptance and remaining limits](../../../docs/Deployment/POS-Cashier-Acceptance.md) for the test commands, live acceptance procedure, supported window size, and the distinction between protected client replay and incomplete server workflow recovery. Joined SQLite-backed local OIDC/cash checkout passed on 2026-09-10. Multi-store portfolio views, exports, reviewer display-name resolution, a Reporting projection, durable offline supervisor commands, and physical-terminal review acceptance remain open.
