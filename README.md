...
The Bangkok manual-tender backend is operational behind Order migration 5, Authorization migration 6, and Reporting migration 14. It provides a branch-authorized, concurrency-safe endpoint and immutable ledger for exact-total THB cash and manually receipt-confirmed PromptPay. Paid transition, settlement, integration event, and audit outbox commit atomically. The event is the future idempotent POS cash-session boundary; POS consumption and the cashier Paid control remain the next slice.
...
Payment authorization, capture, and pre-capture void recovery distinguish definitive outcomes from uncertainty and default to the fail-closed `Disabled` provider. Order migration 5 now exposes and persists exact-total THB cash and manually receipt-confirmed PromptPay with tenant/branch, terminal/operator/idempotency, state, and provider-race invariants. Authorization migration 6 and Reporting migration 14 complete the permission and audit vocabulary. The safe event is ready for future idempotent POS cash-session consumption. POS UI/consumption, provider-specific live evidence, refunds, and financial settlement remain planned.
