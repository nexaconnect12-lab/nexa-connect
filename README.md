...
Payment authorization distinguishes authorized, declined, and unknown outcomes and supports immediate Order-owned capture. Authorization and capture recovery reconcile uncertainty through bounded provider status lookup without spanning provider calls with database transactions. Payment migration 6 publishes transactional capture reconciliation; Order migration 2 consumes it durably, completes only confirmed capture, compensates definitive failure, and retains Inventory/Kitchen work for unknown or exhausted recovery. Reporting migration 11 accepts its audit vocabulary. Sixteen coordinated PostgreSQL 17/RabbitMQ 4 cases passed locally on 2026-08-28. Safety-gated harnesses additionally passed child-process capture recovery, full RabbitMQ Compose restart, isolated Alertmanager firing/resolution delivery, and disposable Payment/Order rollback-forward lifecycles. The provider and alert receivers are local fixtures. Concrete-provider/TLS/rate-limit acceptance, production paging/escalation, and live-traffic rollback rehearsal remain gates. The void/reversal boundary permits only the trusted Order workload to void authorized, uncaptured intents, separates provider failure from uncertainty, and rejects captured payments. Order void consumption, refunds, and financial settlement remain planned.

Durable void recovery is now implemented through Payment migration 7: PostgreSQL leases and bounded status reconciliation never repeat an uncertain provider void, lifecycle/audit events commit through the transactional outbox, and Reporting migration 12 accepts the vocabulary. Local PostgreSQL evidence passed successful transactional recovery and `0→7→6→7`. Order consumption of void events, concrete-provider acceptance, dedicated void telemetry/alerts, refunds, and settlement remain next.
