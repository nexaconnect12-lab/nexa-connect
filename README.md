...
Payment authorization distinguishes authorized, declined, and unknown outcomes. Exact Order workload authorization commits a recoverable lease before provider I/O; timeouts remain unknown, and a background worker reconciles them using the stable Payment intent identifier. Order retains Inventory/Kitchen work in `payment_pending` until reconciliation authoritatively authorizes or fails. Opt-in PostgreSQL, migration, Reporting replay, provider-stub, and forced-recovery evidence remain release gates; capture, void, refunds, and financial settlement remain planned.
