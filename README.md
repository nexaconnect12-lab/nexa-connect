...
The WPF POS separates Checkout, Payment, Shift & cash, and Terminal & sync, with touch product tiles, filtering, explicit totals, terminal-bound SQLite recovery, and a guarded joined verifier. Shift & cash now reads the authoritative cashier/terminal cash reconciliation, shows opening cash, signed movements, expected cash, counted variance, and movement history, then closes only against the reviewed concurrency version. Terminal & sync shows unresolved operation state without exposing protected payloads. Joined SQLite-backed local OIDC/WPF cash checkout passed on 2026-09-10. Server-side intermediate-workflow recovery, reconciliation history/supervisor sign-off, and broader synchronization remain open. See the [POS cashier acceptance guide](docs/Deployment/POS-Cashier-Acceptance.md).
