...
The joined harness now has a test-only [service-owned fixture provisioner](src/Tools/NexaConnect.PaymentReviewAcceptance/README.md) and a [disposable live launcher](docker/payment-review-joined/README.md). The launcher owns fresh PostgreSQL/Keycloak/RabbitMQ/Toxiproxy containers, applies four service migrations, creates reader/resolver identities and five marked review aggregates, starts seven loopback application hosts, routes Order-to-Inventory through a run-scoped proxy, requires all seven browser scenarios, emits sanitized evidence, and cleans up processes and Compose resources.
