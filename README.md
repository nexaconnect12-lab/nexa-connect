...
The WPF POS separates Checkout, Payment, Shift & cash, Cash review, and Terminal & sync, with touch product tiles, filtering, explicit totals, terminal-bound SQLite recovery, and guarded live-evidence verifiers. Active PKCE sessions silently renew short-lived credentials, lock after five minutes without key/click/touch input, and force interactive sign-in at idle or the ten-hour absolute boundary without discarding operational recovery. Shift & cash uses authoritative cashier/terminal reconciliation and a version-fenced close. Cash review adds exact-store closed-session history, movement detail, read-only accountant access, and idempotent/version-fenced investigating and approval decisions for supervisors. SQLite schema 2 persists an exact pending supervisor request before network send and restores it as a locked verification action after restart; late financial movements supersede prior review. Joined SQLite-backed local OIDC/WPF cash checkout passed on 2026-09-10, and the guarded local human Cash Review workflow passed on 2026-09-15. Order migration 6 provides server-side, lease-fenced recovery of interrupted manual-tender workflows through KitchenAccepted; its guarded real-host process-interruption acceptance passed on 2026-09-15. Fresh physical-terminal session-lock and supervisor acceptance, exports/Reporting projection, provider-payment workflow recovery, offline supervisor authorization/execution, and broader synchronization remain open. See the [POS cashier acceptance guide](docs/Deployment/POS-Cashier-Acceptance.md).

The Bangkok manual-tender path uses Order migrations 5-6, Authorization migration 6, Reporting migration 14, and POS migration 4. The WPF cashier client places `cash_manual` or `promptpay_manual` orders without provider capture, requires an explicit Paid confirmation, persists the settlement retry identity across restarts, and fails closed when a PromptPay QR is not configured. Order migration 6 retains the original workflow context and lets a background worker reclaim interrupted Submitted or InventoryReserved work with fenced leases and the original Order identity. Cash posts to the event-time THB session; PromptPay never changes drawer totals. The upgraded SQLite protected-client/PostgreSQL/RabbitMQ/migration runner passed 17/17 on 2026-09-10, followed by joined SQLite-backed local OIDC/WPF cash checkout acceptance.

Payment Review's expanded operations matrix passed **13/13** in a generated, isolated Compose environment. The separate [joined live operator harness](src/Frontend/e2e/payment-review-live/README.md) passed **10/10** locally on 2026-09-02, including Inventory transport outage, Inventory/Kitchen process loss, combined process loss, and explicit recovery without automatic decision replay. See the [release checklist](docs/Deployment/Payment-Review-Release-Checklist.md).

The joined harness now has a test-only [service-owned fixture provisioner](src/Tools/NexaConnect.PaymentReviewAcceptance/README.md) and a [disposable live launcher](docker/payment-review-joined/README.md). The launcher owns fresh PostgreSQL/Keycloak/RabbitMQ/Toxiproxy containers, applies four service migrations, creates reader/resolver identities and eight marked review aggregates, starts seven loopback application hosts, routes Order-to-Inventory through a run-scoped proxy, supervises only the disposable Inventory/Kitchen processes through a token-protected loopback controller, requires all ten browser scenarios, emits sanitized evidence, and cleans up processes and Compose resources.
...
Payment authorization, capture, and pre-capture void recovery distinguish definitive outcomes from uncertainty and default to the fail-closed `Disabled` provider. Order migration 5 exposes and persists exact-total THB cash and manually receipt-confirmed PromptPay. Authorization 6 and Reporting 14 complete permission/audit vocabulary; POS migration 4 consumes the safe event idempotently, and the WPF Paid control is implemented. Joined client-to-service evidence, provider-specific live evidence, refunds, and financial settlement remain planned.

Durable void recovery is now implemented through Payment migration 7: PostgreSQL leases and bounded status reconciliation never repeat an uncertain provider void, lifecycle/audit events commit through the transactional outbox, and Reporting migration 12 accepts the vocabulary. Order migration 3 consumes definitive/uncertain void events through its durable inbox, protects paid orders, compensates confirmed voids, and introduces guarded `payment_review`. Local PostgreSQL evidence passed Payment `0→7→6→7` and Order `0→3→2→3`. Concrete-provider acceptance, live RabbitMQ void replay, dedicated void telemetry/alerts, refunds, and settlement remain next.

Order migration 4 closes the operator payment-review loop. Tenant-authorized operators can list/read cases and resolve them with optimistic concurrency as `confirm_void`, `resume_payment`, or `escalate`. Resolution commits the Order transition, append-only decision history, `order.payment-review-resolved.v1`, and safe `order.audit.v1` together; confirmed void compensation remains retry-at-least-once. Authorization migration 5 preserves dedicated read/resolve grants for roles created after migration and adds accountant read-only access, while Reporting migration 13 accepts the resolution audit vocabulary. The acceptance matrix covers Order `0→4→3→4`, transactional resolution and downgrade refusal, Reporting-13 removal/replay, repository-produced contract publication after a hosted-dispatcher restart, hosted Reporting projection with duplicate suppression, and isolated firing/resolved alert delivery. Run `scripts/test-payment-review-operations.ps1` against each verified disposable release-environment target; its evidence label is descriptive and does not validate infrastructure identity.

The centralized observability foundation provides structured JSON console logs and optional OTLP export through an OpenTelemetry Collector. Local logs flow to Loki, local metrics persist and rules are evaluated in Prometheus, and Alertmanager routes resulting alerts; Grafana provisions both data sources and traces remain debug-only. Local Alertmanager has no external notification receiver. See the [Observability Guide](docs/Deployment/Observability.md) and [ADR-007](docs/Architecture/Decisions/ADR-007-centralized-observability-foundation.md).
