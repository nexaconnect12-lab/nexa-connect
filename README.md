...
Payment authorization distinguishes authorized, declined, and unknown outcomes and supports immediate Order-owned capture. Authorization recovery reconciles unknown outcomes; capture records `captured`, `failed`, or `capture_unknown` without spanning provider calls with database transactions. Order becomes paid only after capture and retains Inventory/Kitchen work for uncertainty. Opt-in PostgreSQL, migration, Reporting replay, provider-stub, capture-recovery, and forced-recovery evidence remain release gates; void, refunds, and financial settlement remain planned.
