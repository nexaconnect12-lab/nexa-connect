...
The Bangkok manual-tender path uses Order migration 5, Authorization migration 6, Reporting migration 14, and POS migration 4. The optional durable POS consumer projects each event once. Cash posts to the event-time THB session, including historical reconciliation after close; PromptPay never changes drawer totals. The cashier Paid control and execution of the checked-in live acceptance matrix remain next.
...
Payment authorization, capture, and pre-capture void recovery distinguish definitive outcomes from uncertainty and default to the fail-closed `Disabled` provider. Order migration 5 exposes and persists exact-total THB cash and manually receipt-confirmed PromptPay. Authorization 6 and Reporting 14 complete permission/audit vocabulary; POS migration 4 consumes the safe event idempotently. POS UI, provider-specific live evidence, refunds, and financial settlement remain planned.
