...
The WPF POS separates Checkout, Payment, Shift & cash, Cash review, and Terminal & sync, with touch product tiles, filtering, explicit totals, terminal-bound SQLite recovery, and a guarded joined verifier. Shift & cash uses authoritative cashier/terminal reconciliation and a version-fenced close. Cash review adds exact-store closed-session history, movement detail, read-only accountant access, and idempotent/version-fenced investigating and approval decisions for supervisors. Late financial movements supersede prior review. Joined SQLite-backed local OIDC/WPF cash checkout passed on 2026-09-10; fresh physical-terminal supervisor acceptance, exports/Reporting projection, server-side intermediate-workflow recovery, and broader synchronization remain open. See the [POS cashier acceptance guide](docs/Deployment/POS-Cashier-Acceptance.md).
