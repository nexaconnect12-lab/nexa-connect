...
Payment authorization distinguishes authorized, declined, and unknown outcomes and supports immediate Order-owned capture. Authorization and capture recovery reconcile uncertainty through bounded provider status lookup without spanning provider calls with database transactions. Payment migration 6 publishes transactional capture reconciliation; Order migration 2 consumes it durably, completes only confirmed capture, compensates definitive failure, and retains Inventory/Kitchen work for unknown or exhausted recovery. Reporting migration 11 accepts its audit vocabulary. Sixteen coordinated PostgreSQL 17/RabbitMQ 4 cases passed locally on 2026-08-28, including Payment `0→6→5→6`, Order `0→2→1→2`, Reporting replay, persisted restart recovery, a discarded provider response before local completion, and replacement of an established publisher connection after closure. Payment/Order expose safe recovery, inbox, and outbox backlog/age gauges; local Prometheus scrapes RabbitMQ and evaluates queue/dead-letter alerts. A safety-gated release script repeats the disposable acceptance matrix, and capture recovery can be paused independently during a controlled provider incident. A real provider-connected process kill, production alert-delivery rehearsal, and release-environment rollback/forward recovery remain gates; void, refunds, and financial settlement remain planned.
