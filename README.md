...
Payment authorization distinguishes authorized, declined, and unknown outcomes and supports immediate Order-owned capture. Capture and pre-capture void recovery reconcile uncertainty through bounded provider status lookup without spanning provider calls with database transactions. Payment migrations 6-7 publish transactional reconciliation; Order migrations 2-3 consume it durably, validate organization/payment-intent ownership, complete only confirmed capture, compensate definitive failure or confirmed void, retain work during uncertainty, and route terminal void problems to payment review. Reporting migrations 11-12 accept the audit vocabulary. Local recovery and rollback-forward evidence passed. Payment now defaults to a fail-closed `Disabled` provider that makes no network calls or recovery claims until a provider is selected; `GenericHttp` must be explicitly configured. Order now contains the domain foundation for exact-amount THB `cash` and manually receipt-confirmed PromptPay settlement, including tenant/branch, terminal/operator/idempotency, state, and provider-race invariants; it is not exposed or persisted yet. The HTTP provider boundary enforces an HTTPS base URL, supports configured secret-injected bearer authentication, applies bounded timeouts and stable capture/void idempotency, and conservatively handles rate limits, malformed responses, and transport uncertainty. An opt-in live-sandbox runner verifies concrete TLS, acceptance of the configured credential, and capture/void replay and status behavior using distinct disposable provider references. The next manual-tender slice must add atomic persistence/audit, authorization, cash-session posting, API/BFF and POS UI. Provider execution, explicit invalid-credential rejection, rate-limit/lost-response evidence, live void messaging, production paging/escalation, and live-traffic rollback remain gates. Refunds and financial settlement remain planned.
