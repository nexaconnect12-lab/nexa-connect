...
The WPF POS now separates Checkout, Payment, Shift & cash, and Terminal & sync, with touch product tiles, name/station filtering, quantity controls, and explicit currency totals. A terminal-bound, schema-versioned SQLite database now stores DPAPI-protected shift, cash, checkout, settlement, and outbox payloads with WAL/FULL durability, explicit retry states, interrupted-send recovery, and legacy-file migration. The client routes tenant-scoped menu reads directly to Catalog and retains the original protected placement command for identical restart/retry. The checked checkout launcher supervises the local service graph, and a guarded local verifier records sanitized evidence only after the selected Order is completed, its linked financial records agree, SQLite has no unresolved operational rows, and credentials are cleared. The joined local OIDC/WPF cash-checkout acceptance passed before the SQLite upgrade on 2026-09-10; the upgraded persistence gate requires a fresh run. Server-side intermediate-workflow recovery remains open. See the [POS cashier acceptance guide](docs/Deployment/POS-Cashier-Acceptance.md).
