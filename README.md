...
Payment authorization is implemented as a development Phase 10 slice: only the exact Order workload can authorize an intent, a concurrency-controlled lease is committed before provider I/O, and a separate transaction records a sanitized authorized/failed result plus audit and outbox events. Order requires `authorized` and no longer accepts `pending` as payment completion. Opt-in PostgreSQL, migration, Reporting replay, and provider-environment evidence remain release gates; capture, void, refunds, reconciliation, and abandoned-lease recovery remain planned.
