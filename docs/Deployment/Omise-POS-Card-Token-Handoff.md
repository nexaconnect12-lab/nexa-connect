...
The [five-scenario hosted gate passed on 2026-09-18](../Architecture/Evidence/Omise-Five-Scenario-Recovery-Acceptance.md). Signed test notifications are a separate optional Payment migration-8 capability; enable only after following [webhook prerequisites and secret isolation](Omise-Webhooks.md). External webhook delivery acceptance remains pending.
