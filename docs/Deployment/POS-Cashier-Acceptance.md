...
Apply Order migration 7 before starting the updated Order service. Enable `WorkflowRecovery__Enabled=true` only after Catalog, Inventory, Kitchen, and Payment endpoints plus Order workload identity are healthy. Enable `PaymentReconciliationConsumer` with the checkout RabbitMQ secret for provider tenders; `scripts/run-checkout-development.ps1` now supplies both settings automatically. Defaults are a 5-second poll, 30-second lease, and 15-second retry delay. The worker uses the original Order ID and correlation context, uses a stable Payment intent key, and stops manual-tender recovery at KitchenAccepted. Monitor `order.workflow_recovery.pending`, `order.workflow_recovery.oldest_age_seconds`, `order.workflow_recovery.steps`, `order.workflow_recovery.failures`, and reconciliation queue/dead-letter depth; telemetry contains no tenant, Order, or Payment identifiers.
