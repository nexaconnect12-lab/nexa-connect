...
Local service launch scripts do not contain database passwords or workload secrets. Export `ConnectionStrings__Authorization`, `ConnectionStrings__Restaurant`, `ConnectionStrings__POS`, and `WorkloadIdentity__ClientSecret` from the local `.env`/secret store before running `scripts/run-pos-development.ps1`. PostgreSQL outbox mode also requires an explicit `Outbox:ConnectionString`; no RabbitMQ guest credential is used as a fallback.
