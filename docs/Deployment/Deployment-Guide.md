...
Inventory and Kitchen use the shared durable inbox. Reporting migration 3 adds activity storage/inbox and migration 4 expands the accepted product-audit vocabulary. Enable `Outbox__Enabled` plus secret-managed `Outbox__ConnectionString` on participating Platform Directory, Restaurant, Catalog, Media, Notification, and Payment publishers, and `ActivityConsumer__Enabled` plus `ActivityConsumer__ConnectionString` on Reporting. Reporting declares durable queue `nexaconnect.reporting.activity.v1` on `nexaconnect.events` and binds `*.audit.v1`. Roll out Reporting migrations and the compatible consumer before publishers. Roll back publishers first, drain/stop the consumer, disable reads, then downgrade migration 4; its destructive downgrade removes expanded-vocabulary projections and their completed inbox markers so source events must remain available for controlled replay after re-upgrade. Optional HTTP ingestion client/source mappings remain default-deny.
