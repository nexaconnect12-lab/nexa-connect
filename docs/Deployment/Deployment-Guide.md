...
Payment PostgreSQL authorization requires Payment migrations 3-4 and Reporting migrations 8-9 before traffic/publication. New intents require organization context; authorization is Order-workload-only and atomically commits a recoverable `authorizing` lease before provider I/O. Timeouts remain `unknown`; the recovery worker claims expired leases, reconciles through `PaymentProvider__AuthorizationStatusPath`, and commits sanitized results plus audit/outbox events. Configure `PaymentProvider__BaseUrl`, `PaymentProvider__AuthorizationPath`, and `PaymentProvider__AuthorizationStatusPath` from `docs/Deployment/production.env.example` through reviewed environment settings and never log or persist provider bodies, PAN, CVV, tokens, or secrets. Migration 2 legacy ownership backfill remains mandatory. For rollback, stop Order payment traffic, disable publishers, drain dispatch, reconcile all `authorizing`/`unknown`/`requires_action`/`authorized` rows and provider references, downgrade Reporting 9, then downgrade Payment 4→3. Continue with destructive Payment 3→2→1 only when intentionally removing authorization/organization ownership and after verifying no restaurant/idempotency key spans organizations. Provider recovery evidence is a release blocker.
...
Inventory and Kitchen use the shared durable inbox. Reporting migration 3 adds activity storage/inbox, migration 4 expands the accepted product-audit vocabulary, migration 5 adds Kitchen audit vocabulary, migration 6 adds Customer profile audit vocabulary, migration 7 adds Notification delivery lifecycle vocabulary, migration 8 adds Payment authorization lifecycle vocabulary, and migration 9 adds uncertainty/reconciliation vocabulary. Enable `Outbox__Enabled` plus secret-managed `Outbox__ConnectionString` on participating Platform Directory, Restaurant, Catalog, Media, Notification, Kitchen, Customer, and Payment publishers, and `ActivityConsumer__Enabled` plus `ActivityConsumer__ConnectionString` on Reporting. Reporting declares durable queue `nexaconnect.reporting.activity.v1` on `nexaconnect.events` and binds `*.audit.v1`. Roll out compatible Reporting migrations/consumer before publishers. Roll back publishers first, drain/stop the consumer, disable reads, then downgrade; destructive vocabulary downgrades remove incompatible projections and completed inbox markers so source events must remain available for controlled replay after re-upgrade. Optional HTTP ingestion client/source mappings remain default-deny.
...
Payment's equivalent opt-in recovery acceptance uses `NEXACONNECT_PAYMENT_INTEGRATION_DB` with the shared RabbitMQ variables and an isolated schema/exchange/queue. It includes authorization lease/result atomicity; the updated clean-install case runs 0→4→3→4 in generated `nexaconnect_payment_clean_it_<guid>` databases. Provider-stub, forced-termination, and established-dispatcher recovery evidence remain release gates. Never run acceptance against production infrastructure.
Enable Order's `PaymentReconciliationConsumer` only after Order migrations, Reporting compatibility, RabbitMQ, PostgreSQL inbox access, and idempotent Inventory/Kitchen compensation are verified. It consumes `payment.authorization-reconciled.v1` through `PaymentReconciliationConsumer:Queue`, requires PostgreSQL Order persistence and HTTP workflow adapters, and uses the Order-owned durable inbox. Keep it disabled during rollback and drain the queue before downgrading.
