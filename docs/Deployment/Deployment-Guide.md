...
Inventory and Kitchen expose the shared durable inbox store when PostgreSQL persistence is enabled. A message is claimed with a lease, marked completed only after the handler succeeds, and returned to the retryable queue after handler failure or lease expiry. Reporting migration 3 adds the activity projection. Configure indexed `ActivityProjection__Clients__{client-id}__SourceService` entries with dedicated audit-projector workload client IDs; no client/source mappings disables ingestion. Source publication and RabbitMQ consumption remain future work, so activity stays empty until a trusted publisher calls the internal contract. Disable activity routes before downgrading migration 3. Apply the latest service migrations before enabling durable consumers; do not treat RabbitMQ acknowledgement alone as idempotency.
