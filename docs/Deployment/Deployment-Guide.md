...
Customer BFF notification reads require `Services__Notification`; the BFF derives organization and application headers from its protected tenant cookie. Notification durable integration requires Notification migration 3, Authorization migration 2, `Persistence__Provider=PostgreSQL`, `ConnectionStrings__Notification`, `Services__PlatformDirectory`, `Services__Authorization`, and secret-managed `Outbox__ConnectionString`. Enable `NotificationConsumer__Enabled` only after the schema is current; its default durable queue is `nexaconnect.notification.requested.v1`, with permanent failures routed by `notification.requested.v1.dead`. Provider lifecycle audit projection requires Reporting migration 7 before delivery publication. Roll back producers and delivery first, stop or drain the consumer/outbox publisher, then downgrade Reporting and Notification. Never log notification recipient/subject/body, provider payloads/responses, or broker/provider credentials.
...
Customer profile publication requires Customer migration 2, Reporting migration 6 with its compatible consumer, `Persistence__Provider=PostgreSQL`, `ConnectionStrings__Customer`, `Services__PlatformDirectory`, and `Services__Authorization`. Enable `Outbox__Enabled` only after Reporting is compatible and provide secret-managed `Outbox__ConnectionString`; Customer publishes `customer.profile-created.v1` and `customer.audit.v1`. Stop Customer publication and drain/stop Reporting before destructive rollback. Customer 2→1 removes only audit objects and preserves migration-1 profile/outbox state; Reporting 6→5 removes Customer projections and completed inbox markers, so retain source events for controlled replay. Never log names, identity subjects, contacts, addresses, preferences, request bodies, or event payloads.

Payment authorization, capture, and recovery require Payment migrations 3-6, Order migration 2, and Reporting migrations 8-11 before traffic/publication. Authorization and capture are exact-Order-workload-only and commit local start state before provider I/O. Capture sends the Payment intent ID in `Idempotency-Key`; recovery performs only `PaymentProvider__CaptureStatusPath` lookup with that stable identity and never repeats the capture command. Configure the provider base, authorization/capture command and status paths, lease duration, recovery interval, and maximum capture recovery attempts from `production.env.example`. Unknown or exhausted results remain payment-pending; only explicit capture permits completion and only explicit failure permits compensation. Never log or persist provider bodies, PAN, CVV, tokens, secrets, or unrestricted personal data. Migration 2 legacy ownership backfill remains mandatory. Payment 6→5 is blocked while capture recovery state remains; Order 2→1 is blocked while `payment_pending` orders remain; Payment 5→4 retains its capture-state guard. Stop Order payment traffic and recovery workers, disable publishers/consumers, drain dispatch, reconcile provider/accounting state, retain source events, and take a verified backup before reviewed downgrades. Local database, migration, replay, new-connection broker recovery, and rollback-path evidence passed on 2026-08-26; production-provider process termination, established-connection recovery, alert delivery, and release-environment rollback/forward-recovery rehearsal remain release blockers.

The Kitchen API is exposed at `/api/kitchen/v1/tickets` for authenticated Order workloads. Configure `ConnectionStrings__Kitchen` and `Persistence__Provider=PostgreSQL` with the service-owned Kitchen database. Its HTTPS endpoint must be the value of Order's `Services__Kitchen` setting.

Kitchen migration 3 removes the static restaurant setting from new writes: Order propagates organization/restaurant/branch and only its exact workload identity may create/cancel. Configure `Services__PlatformDirectory`, `Services__Restaurant`, `Services__Authorization`, `WorkloadIdentity__Authority`, `WorkloadIdentity__ClientId=nexaconnect-kitchen-service`, secret-managed `WorkloadIdentity__ClientSecret`, and observability settings for operator reads/transitions. Apply Authorization migration 3 before operator traffic and reconcile legacy empty-organization rows from Order before traffic. Deploy Reporting migration 5 plus its compatible consumer before enabling Kitchen `Outbox__Enabled`; rollback publishers/consumer first, drain dispatch, then perform destructive 3→2. Kitchen emits `kitchen.ticket-queued.v1`, `kitchen.ticket-status-changed.v1`, and `kitchen.audit.v1`.
...
POS cash-replay acceptance requires `NEXACONNECT_POS_INTEGRATION_DB` and `NEXACONNECT_ENVIRONMENT=Testing` (or another Development/Test value). It creates uniquely named schemas in the configured non-production PostgreSQL database and removes them after each case. Seven PostgreSQL 17 cases passed locally. Use a role permitted to create/drop schemas, never production credentials; without the opt-in connection/environment the tests report skipped.

The full POS migration acceptance additionally requires `NEXACONNECT_POS_CLEAN_INSTALL_ACCEPTANCE=1` and `NEXACONNECT_POSTGRES_ADMIN_INTEGRATION_DB` for a disposable administrator allowed to create/drop databases. It manages only generated `nexaconnect_pos_clean_it_<guid>` names and invokes the actual runner for 0→3→2→3, including safe migration-3 downgrade/re-upgrade and repository verification. The lifecycle passed locally against PostgreSQL 17 and removed its generated database. Never grant `CREATEDB` to runtime or normal migration identities and never use production credentials.

Operational provider settings: Order outbound calls use `Authentication__TokenEndpoint`, `Authentication__ClientId`, and `Authentication__ClientSecret` to obtain short-lived Keycloak client-credentials tokens (a static `Authentication__OutboundToken` is for development only). Payment uses bounded retries for transient 5xx/timeout responses. Notification provider delivery requires PostgreSQL plus `NotificationDelivery__Enabled=true`, HTTPS `NotificationProvider__BaseUrl`, secret-managed `NotificationProvider__ApiToken`, bounded `NotificationProvider__ProviderCode`, and submit/receipt paths. `NotificationDelivery__MaximumAttempts`, `PollInterval`, and `Lease` bound processing; notification ID is the provider idempotency/client reference. HTTP `408`/`429`/`5xx` and invalid responses retry, `400`/`422` fail permanently, and accepted receipts are polled until delivered or exhausted. Keep delivery disabled when no compatible provider contract is deployed; PostgreSQL remains authoritative in all cases.

Inventory and Kitchen use the shared durable inbox. Reporting migration 3 adds activity storage/inbox; migrations 4-10 extend approved product lifecycle vocabulary; migration 11 adds Payment capture-reconciliation audit vocabulary. Enable `Outbox__Enabled` plus secret-managed `Outbox__ConnectionString` on participating publishers, and `ActivityConsumer__Enabled` plus `ActivityConsumer__ConnectionString` on Reporting. Reporting declares durable queue `nexaconnect.reporting.activity.v1` on `nexaconnect.events` and binds `*.audit.v1`. Roll out compatible Reporting migrations/consumer before publishers. Roll back publishers first, drain/stop the consumer, disable reads, then downgrade; destructive vocabulary downgrades remove incompatible projections and completed inbox markers so source events must remain available for controlled replay after re-upgrade. Optional HTTP ingestion client/source mappings remain default-deny.
...
Payment's opt-in recovery acceptance uses `NEXACONNECT_PAYMENT_INTEGRATION_DB` with the shared RabbitMQ variables and an isolated schema/exchange/queue. It covers authorization/capture atomicity and recovery state; the clean-install case targets 0→6→5→6 in generated `nexaconnect_payment_clean_it_<guid>` databases. Focused automated coverage proves duplicate terminal delivery and partial-compensation redelivery, and the coordinated local matrix includes Reporting replay. Production-provider delayed/dropped responses and process termination, established-dispatcher recovery, and alert delivery remain release gates. Never run acceptance against production infrastructure.

The coordinated local PostgreSQL 17/RabbitMQ 4 matrix passed 14 tests on 2026-08-26, including Payment `0→6→5→6`, Order `0→2→1→2`, Reporting migration-11 replay, persisted uncertainty/restart recovery, transactional reconciliation publication, and broker recovery over a new connection. See the [Payment capture-recovery evidence](../Architecture/Evidence/Phase-11-Payment-Capture-Recovery.md) and [operator runbook](Payment-Capture-Recovery-Runbook.md). Production-provider process termination, established-connection recovery, and alert-delivery rehearsal remain environment-specific gates.

Enable Order's `PaymentReconciliationConsumer` only after Order migration 2, Reporting compatibility, RabbitMQ, PostgreSQL inbox access, and idempotent Inventory/Kitchen compensation are verified. It consumes `payment.authorization-reconciled.v1` and `payment.capture-reconciled.v1` through `PaymentReconciliationConsumer:Queue`, requires PostgreSQL Order persistence and HTTP workflow adapters, and uses the Order-owned durable inbox. Keep it disabled during rollback and drain the queue before downgrading.
