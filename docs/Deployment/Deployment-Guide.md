...
The current Media write lifecycle is development-only. Production requires a rotated bucket/prefix-scoped identity over TLS, object-access audit logs, an HTTPS browser endpoint, and explicit portal-origin CORS. Current local Compose uses MinIO root credentials and does not install bucket CORS; do not enable untrusted writes until owner validation, quarantine/scanning, quotas, cleanup, and reconciliation exist.

For Media, apply migrations 1 then 2 and configure `MediaStorage__ServiceUrl/Bucket/AccessKey/SecretKey` from secret management. Keep the bucket private and configure CORS for the portal origin to PUT `Content-Type` and `x-amz-meta-sha256`; credentials never reach the browser. Local Compose exposes MinIO at 9100/9101 and creates `nexaconnect-media`. Verify start/PUT/complete/download/delete before enabling the UI and drain pending sessions before downgrading migration 2.
