...
Payment's opt-in recovery acceptance uses `NEXACONNECT_PAYMENT_INTEGRATION_DB` with the shared RabbitMQ variables and an isolated schema/exchange/queue. It covers authorization/capture atomicity and recovery state; the clean-install case targets 0→6→5→6 in generated `nexaconnect_payment_clean_it_<guid>` databases. Provider-stub, forced-termination, Order duplicate/redelivery, Reporting replay, and established-dispatcher recovery evidence remain release gates. Never run acceptance against production infrastructure.
