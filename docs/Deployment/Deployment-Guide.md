...
Payment PostgreSQL authorization requires Payment migration 3 and Reporting migration 8 before traffic/publication. New intents require organization context; authorization is Order-workload-only and atomically commits an `authorizing` lease before provider I/O, then a sanitized result plus audit/outbox events in a separate transaction. Configure `PaymentProvider__BaseUrl` and `PaymentProvider__AuthorizationPath` from `docs/Deployment/production.env.example` through reviewed environment settings and never log or persist provider bodies, PAN, CVV, tokens, or secrets. Migration 2 legacy ownership backfill remains mandatory. For rollback, stop Order payment traffic, disable publishers, drain dispatch, reconcile all `authorizing`/`authorized` rows and provider references, downgrade Reporting 8, then downgrade Payment 3→2. Continue with destructive Payment 2→1 only when intentionally removing organization/audit ownership and after verifying no restaurant/idempotency key spans organizations; otherwise stop at version 2. Abandoned-lease recovery and provider reconciliation are release blockers for production authorization enablement.
...
Payment's equivalent opt-in recovery acceptance uses `NEXACONNECT_PAYMENT_INTEGRATION_DB` with the shared RabbitMQ variables and an isolated schema/exchange/queue. It now includes authorization lease/result atomicity. `NEXACONNECT_PAYMENT_CLEAN_INSTALL_ACCEPTANCE=1` plus a disposable administrator runs 0→3→1→3 only in generated `nexaconnect_payment_clean_it_<guid>` databases. Neither case uses provider credentials, proves abandoned-lease recovery, or proves established-dispatcher reconnection. Never run either against production infrastructure.
