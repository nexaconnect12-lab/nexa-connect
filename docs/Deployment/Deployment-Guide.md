...
Payment PostgreSQL mode requires migration 2 before traffic. New intents require organization context and atomically persist append-only audit plus `payment.intent-created.v1` and `payment.audit.v1`; enable dispatch with `Outbox__Enabled=true` and secret-managed `Outbox__ConnectionString`. Deploy Reporting migration 4 and the compatible Reporting consumer binary before enabling Payment dispatch. Order's Payment adapter propagates organization, restaurant, and branch from the Order aggregate, and Payment allows only the exact Order workload to bypass customer authorization. Migration 2 marks legacy intents with the empty organization UUID: reconcile each row against authoritative Order ownership and prove no sentinel remains before application rollout. For rollback, disable producers, drain/stop dispatch, verify no restaurant/idempotency key spans organizations, roll back the application, then perform destructive 2→1; it preserves intents/outbox rows but removes organization attribution and audit history.
...
Payment's equivalent opt-in recovery acceptance uses `NEXACONNECT_PAYMENT_INTEGRATION_DB` with the shared RabbitMQ variables and an isolated schema/exchange/queue. `NEXACONNECT_PAYMENT_CLEAN_INSTALL_ACCEPTANCE=1` plus a disposable administrator runs 0→2→1→2 only in generated `nexaconnect_payment_clean_it_<guid>` databases. Neither case uses provider credentials or proves established-dispatcher reconnection. Never run either against production infrastructure.
