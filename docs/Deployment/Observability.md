...
### Omise webhook worker

The Payment worker uses service/ActivitySource `nexaconnect-payment`, exports the bounded `payment.omise_webhook.process` Consumer span and `payment.omise_webhook.outcomes` counter with outcome tags, and scopes JSON logs with correlation and trace IDs. Incoming validated correlation strings survive durable ingestion and outbound propagation; financial events reuse an incoming UUID or generate a separate UUID bridge. Automatic HTTP spans are suppressed while processing canonical event and current-charge reads because their URLs contain provider references. No event/charge IDs, bodies, signature headers or secrets belong in telemetry. In Loki, query `{service_name="nexaconnect-payment"} |= "Omise webhook"`; inspect outcome metrics through the configured OTLP pipeline. Local traces still have only the debug exporter described above. See [webhook operations](Omise-Webhooks.md).

Cash-close monitoring adds POS publication outcomes, unpublished snapshot count/age and collection-failure telemetry, plus Reporting applied/replayed/rejected/retry/connection-retry outcomes. Backlog polling starts with either POS publication or outbox enabled, samples every 15 seconds and retains last successful gauges on failure. Six rules in `docker/prometheus/rules/cash-close-reporting.yaml` cover outbox stalls, collection failure, default Reporting queues/dead letters and retries; queue rules need RabbitMQ metrics and customized names when deployments differ. These are development thresholds, not verified alert delivery or completeness measures. The replay CLI uses `nexaconnect-cash-close-replay` JSON logs and optional `OTEL_EXPORTER_OTLP_ENDPOINT` log export. See [metrics, queries and response procedure](Cash-Close-Recovery.md#monitoring).

`scripts/test-cash-close-alerts.ps1` now validates all six cash-close rules with Prometheus 3.5.0 using healthy, pending, firing, resolved and exact-threshold fixtures. Local checks passed on 2026-09-24. The authored PR/manual workflow runs this gate before recovery, but remote CI and receiver delivery remain unverified. See [evidence](../Architecture/Evidence/Cash-Close-Recovery-Acceptance.md).
