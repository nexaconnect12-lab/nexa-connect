...
Payment exposes anonymous `GET /health/live` for process liveness and `GET /health/ready` for traffic readiness. PostgreSQL mode is ready only when the database is reachable and migration 7 or newer is present. Provider availability is deliberately excluded: provider incidents must surface through recovery metrics and alerts rather than cause orchestrator restart loops.
...
Payment migration `7→6` is not currently an executable operational rollback after any void activity. Its guard rejects unresolved and historical void state, provider references, attempts, and reconciliation timestamps, and the repository has no approved archival/removal tool. Prefer forward recovery; never manually clear financial state to bypass the guard. A future reviewed archive/export-and-delete mechanism must preserve provider/accounting evidence and auditability before physical downgrade can be supported.
...
For concrete-provider evidence, inject the sandbox URL, API key, amount/currency, and two distinct disposable provider authorization references described in the Payment service README. Run `scripts/test-payment-provider-live-sandbox.ps1 -ConfirmSandboxTransactions`. Do not reuse customer, production, already-captured, or already-voided references. The script rejects HTTP and production-looking hosts, URL credentials/query/fragment, and duplicate references. Archive its boolean-only `.runstate/payment-provider-live/<run-id>/summary.json`; treat TRX and failed output as restricted diagnostics because endpoint details may appear. A passing run closes TLS, configured-credential acceptance, and command/status idempotency for that account only; invalid-credential rejection, rate limits, credential rotation, and lost-response behavior still require provider-specific controlled tests.
