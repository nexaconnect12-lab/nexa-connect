...
Run the opt-in Payment/Order/Reporting acceptance matrix against disposable databases and isolated RabbitMQ resources. Required evidence includes Payment `0→6→5→6`, Order `0→2→1→2`, Reporting migration-11 removal/replay, persisted uncertainty followed by recovery, audit/outbox atomicity, duplicate delivery, partial-compensation retry, and resource cleanup. Focused automation also closes an established RabbitMQ publisher connection and proves the transport creates a replacement without losing either persistent delivery. Its provider crash-boundary case deliberately discards a captured response before local completion and reconciles it after reconstructing the repository/service; it does not kill an operating-system process. Repeat a real provider-connected process termination and alert-delivery rehearsal in each release environment.

Set `NEXACONNECT_PAYMENT_INTEGRATION_DB`, `NEXACONNECT_REPORTING_INTEGRATION_DB`, `NEXACONNECT_POSTGRES_ADMIN_INTEGRATION_DB`, and `NEXACONNECT_RABBITMQ_INTEGRATION_URI` through secret injection. The Payment and Reporting identities must be allowed to create and drop isolated schemas; the administrator identity must be allowed to create and drop the acceptance runners' generated databases. Verify every target is disposable and non-production, then run `scripts/test-payment-capture-recovery-release.ps1 -TargetEnvironment Staging -ConfirmDisposableInfrastructure`. The confirmation switch is an operator attestation; the script also rejects missing settings, invalid RabbitMQ URI schemes, and values containing recognizable production markers, but it cannot independently prove infrastructure ownership. It enables the existing safety-gated acceptance flags, runs the coordinated integration and compensation matrices, restores the caller's environment flags, and does not echo connection values itself. `TargetEnvironment` labels the evidence while the test process uses the accepted `Testing` safety value. The focused suite automates established-client connection replacement, but the script does not orchestrate a full broker-container restart or claim the provider-connected process-kill or alert-delivery gates; record those environment exercises separately.
