...
The [cash-close Reporting slice](../API/Cash-Close-Reporting.md) adds POS migration 6 (`cash_close_publications`, per-session financial/review/snapshot checkpoint and `ix_cash_sessions_closed_publication`) and Reporting migration 15 (`cash_close_facts`, `cash_close_event_receipts`, `ix_cash_close_facts_scope_time`). POS locks a session and commits its snapshot outbox record/checkpoint together; Reporting commits the translated snapshot/receipt hash together. No cross-service SQL or permission migration is introduced. POS downgrade refuses after publication history; Reporting downgrade destroys projection data/receipts and requires retained source-event replay after re-upgrade. Both new migrations require application version `0.15.0`; the local PostgreSQL/broker recovery matrix passed on 2026-09-24, but does not certify the full migration runner or production deployment.
...
Order migration 6 replaces the ambiguous pre-payment `accepted` write path with exact `inventory_reserved` and `kitchen_accepted` stages for new workflows. It stores the original payment method and correlation ID and adds a claim ID, lease expiry, next-attempt timestamp, attempt count, and safe error category for recoverable manual-tender orders. Claims use row locking with `SKIP LOCKED`; final progression verifies the unexpired fencing claim and commits aggregate state plus outbox in one transaction. Downgrade refuses while precise-stage data, an active recovery claim, or a submitted recoverable manual-tender workflow exists. Existing legacy `accepted` rows remain valid and are interpreted as KitchenAccepted because their earlier stage cannot be reconstructed safely.

Order migration 7 replaces the partial recovery index so Submitted and InventoryReserved rows with any retained payment method are eligible, while KitchenAccepted is eligible only for provider methods. It adds no mutable payment data: Payment continues to own intent state, provider references, leases, and reconciliation events. The Order claim fences one workflow writer, and the Payment intent's stable `order:{orderId}` idempotency key plus Payment concurrency versions fence provider operations. The 7→6 downgrade only restores the older index and is safe; disable the updated worker first because version-6 application code intentionally ignores provider KitchenAccepted rows.

The test-only Payment Review joined-fixture tool uses four separately owned loopback databases named exactly `nexa_review_it_<32-hex-run-id>_<platform|restaurant|authorization|order>`. The joined infrastructure launcher creates them in a fresh ephemeral PostgreSQL container, applies service migrations with `nexaconnect_migration`, then provisions through least-privilege service data roles. Before its first write, the tool concurrently probes fixture-owned root/state tables through each owning Infrastructure repository and refuses detected prior state. The tool introduces no cross-service tables or transaction; a partial failure retires the complete disposable run. See the [tool guide](../../src/Tools/NexaConnect.PaymentReviewAcceptance/README.md) and [infrastructure guide](../../docker/payment-review-joined/README.md).
...
Payment migration 7 adds sanitized provider void references, recoverable void leases, bounded attempts, reconciliation timestamps, and a downgrade guard that refuses removal while any void lifecycle/recovery state remains. Reporting migration 12 accepts void lifecycle and reconciliation audit vocabulary. Order migration 3 adds persisted `organization_id`/`payment_intent_id` reconciliation ownership, a unique partial ownership index, and database-enforced `payment_review`; its downgrade refuses review rows/history or populated ownership. Legacy organization and pending-order payment-intent values remain nullable for migration compatibility and must be authoritatively backfilled before application access, which fails closed when ownership is missing. Local PostgreSQL evidence passed successful Payment transactional `void_unknown` recovery, Payment `0→7→6→7`, and Order `0→3→2→3`; disposable databases were removed afterward.

Order migration 4 adds `order_payment_reviews` and append-only `order_payment_review_history`. A case is unique by Order and organization/payment intent, open-case reads are organization/branch indexed, and every operator action advances an optimistic concurrency version. Case update, Order transition, immutable history, resolution event, and safe audit outbox commit in one transaction. Downgrade refuses once any case or history exists. Reporting migration 13 accepts the corresponding Order audit vocabulary. The live Order runner passed `0→4→3→4` and transactional repository resolution.

The operator UI reads history through Order's Infrastructure repository using parameterized organization/order predicates, descending concurrency order, and a limit of 100. This uses existing migration-4 objects; no migration or cross-service database access was added. History is authoritative committed Order state, not Reporting's eventually consistent audit projection.

Resolution claims store a two-minute expiry and opaque fencing token. Claim acquisition advances concurrency; finalization must match both version and token. Expired takeover is constrained to the same resolution so compensation cannot race a conflicting resume decision. History retains the Authorization decision ID required by ADR-004.

Authorization migration 5 backfills `order.payment-review.read` for existing accountant roles. Runtime assignment provisioning applies Payment Review read/resolve permissions to newly created tenant-admin/store-manager roles and read only to newly created accountant roles, so clean installs and upgraded databases produce the same authorization model. Migration 5 adds no table or index. Its destructive `5→4` downgrade deletes only accountant/read associations; migration 4 remains the owner of tenant-admin/store-manager Payment Review associations, and `4→3` removes those read/resolve rows.
...
POS cash replay has successful isolated-schema PostgreSQL 17 evidence for exact/concurrent deduplication, rollback, terminal/subject denial, active scope, and shift concurrency. Migration 4 adds the immutable, Order-unique manual-tender projection; cash projection and drawer movement share one transaction while PromptPay creates no movement. Current full acceptance invokes the actual runner for `0→5→4→3→5` in a generated `nexaconnect_pos_clean_it_<guid>` database, validates version-owned objects and retained cash/replay state, exercises real repositories, and verifies migration-5 downgrade refusal after review history. The earlier guarded manual-tender projection/RabbitMQ matrix passed 3/3.

POS migration 5 adds `cash_session_review_states` as the current decision projection and append-only `cash_session_review_history` keyed by client operation UUID. Each history row retains the reviewed financial version, decision, bounded reason, reviewer subject, Authorization decision ID, payload hash, review version, and UTC timestamp. The projection and history commit together under a cash-session row lock. A database trigger rejects history updates/deletes. Late movements advance the cash-session financial version; reads derive `review_required` when the stored reviewed version is stale and variance remains nonzero, or `balanced` when the resulting variance is zero. Downgrade refuses once either review table contains data.

Authorization migration 7 backfills `pos.cash-review.read` and `pos.cash-review.resolve` for existing tenant-admin/store-manager roles and read only for accountant. Runtime assignment provisioning applies the same split to roles created later. The migration adds no table or index; `7→6` removes only these associations.
...
Reporting tables are rebuildable projections. Migration 3 adds bounded `activity_records`, keyed by event ID and indexed for tenant cursor reads; migrations 4-13 expand database-enforced vocabulary through Order payment-review resolution audit contracts. Participating owning services insert local audit plus outbox events atomically. Reporting deduplicates through `inbox_messages` and acknowledges RabbitMQ after handling. Vocabulary downgrades delete incompatible projections and their completed inbox markers, requiring retained source events for controlled replay after re-upgrade. Reporting-13 live projection/replay remains a release gate. Retention/archive and a full replay checkpoint remain unimplemented; broker retention and source outboxes are the current recovery inputs.

## 7. Branch-local data

Order's Inventory reservation/release adapter supplies both tenant headers so trusted calls use organization-scoped keys and transactional audit/outbox persistence. Inventory preserves the legacy unscoped path only when both headers are absent; partial or invalid context fails closed. This routing correction introduces no migration.

The WPF POS now owns schema-2 SQLite local state and cash-movement outbox persistence. Its database is bound to organization/branch/store/terminal scope; DPAPI protects state and command payloads; WAL/FULL durability and explicit queue transitions cover restart and brief service interruption. The former JSON/DPAPI files migrate together only when SQLite is empty. Schema 1 advances transactionally to schema 2 without a table rewrite; older clients reject schema 2 and operators must not lower the version manually. The key-based state now includes an exact pending supervisor decision, reason, terminal scope, idempotency identity, and expected financial/review versions saved before network transmission. Integrity, schema, scope, decryption, or mixed-state failure stops startup and preserves recovery material. Checkout/settlement cleanup is one transaction, and completed outbox acknowledgements remain until retention purge. See [POS local SQLite](POS-Local-SQLite.md) and [cashier recovery boundaries](../Deployment/POS-Cashier-Acceptance.md).

POS cashier reconciliation uses the existing PostgreSQL `cash_sessions`, `cash_movements`, and shift/terminal ownership boundary. A repeatable-read snapshot calculates signed net movement and expected closing cash and returns the matching movement history and version. Each newly committed movement advances `cash_sessions.concurrency_version` in the same transaction; an idempotent replay does not advance it again. Close compares the reviewed version together with the shift's cashier subject and terminal, then stores expected cash, counted cash, variance, closure time, and the incremented version in one update. A late Order cash-settlement projection recalculates both stored expected cash and variance from the authoritative movement set in the same transaction. Cash Review also derives expected cash from those movements, allowing pre-correction closed rows with null or stale stored expected values to remain readable without rewriting audit history. Supervisor history and decisions are the separate migration-5 review aggregate described above.

Cached menu/configuration, offline authorization, order/shift/device synchronization, and kiosk customer-session cleanup remain future local-schema work. The current SQLite boundary does not resume incomplete server workflows.
...
POS migration 7 (`0007_cash_close_replay_audit`, minimum application `0.16.0`) adds a scoped capture-time index over retained cash-close outbox payload metadata, append-only `cash_close_replay_runs` and `cash_close_replay_attempts`. Runs retain scope, UTC range, selection limit, manifest/count, asserted operator UUID and `database_actor=session_user`. Attempts record started/confirmed delivery; replay does not update financial, checkpoint or source-outbox state. Downgrade refuses after any run. See [least privilege and replay recovery](../Deployment/Cash-Close-Recovery.md).

The [joined cash-close portal fixture](../Deployment/Cash-Close-Portal-Acceptance.md) applies existing Platform Directory 3, Restaurant 3, Authorization 7, POS 7 and Reporting 15 migrations to generated disposable databases. It introduces no schema or ownership change. Separate migration and runtime identities provision the graph; fixture-only Infrastructure bootstraps stores and changes an existing read override to deny. Reporting receives data only through the actual consumer.
