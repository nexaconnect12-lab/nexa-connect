...
The WPF cashier also atomically replaces a current-user DPAPI-protected `pending-checkout.bin` before placement HTTP. It retains the order/idempotency identity, settlement key, terminal scope, currency, checkout mode, and product quantities through settlement cleanup. The separate protected settlement record retains exact tender replay fields. Corruption fails closed; this two-file recovery mechanism is not a transactional SQLite outbox or a server workflow resumer. See [cashier recovery boundaries](../Deployment/POS-Cashier-Acceptance.md).
