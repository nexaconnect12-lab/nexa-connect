...
Payment's intent-creation acceptance and Reporting migration-4 vocabulary persistence previously passed locally against PostgreSQL 17 and RabbitMQ. Payment migrations 3-4 and Reporting migrations 8-9 add authorization lifecycle, recoverable lease, uncertainty, and reconciliation persistence/projection; their updated opt-in PostgreSQL authorization and 0→4→3→4 runner cases require fresh release evidence. Production legacy backfill still requires authoritative Order ownership reconciliation, and production authorization additionally requires provider-environment recovery evidence.
