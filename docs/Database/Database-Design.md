...
Payment migrations 3-4 and Reporting migrations 8-9 add authorization lifecycle and reconciliation; Payment migration 5 and Reporting migration 10 add capture state/vocabulary; Payment migration 6, Order migration 2, and Reporting migration 11 add recoverable capture status, durable Order consumption, and reconciliation audit vocabulary. Sixteen coordinated PostgreSQL 17/RabbitMQ 4 cases passed locally on 2026-08-28, including the actual Payment `0→6→5→6`, Order `0→2→1→2`, Reporting replay, persisted restart recovery, deterministic provider-response loss before local completion, and established publisher-connection replacement. Production legacy backfill still requires authoritative Order ownership reconciliation; production-provider OS process termination and alert delivery remain environment-specific release evidence.
