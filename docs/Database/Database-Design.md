...
The test-only Payment Review joined-fixture tool uses four separately owned loopback databases named exactly `nexa_review_it_<32-hex-run-id>_<platform|restaurant|authorization|order>`. Operators create and migrate them before provisioning and must verify every database is fresh. Before its first write, the tool concurrently probes fixture-owned root/state tables through each owning Infrastructure repository and refuses detected prior state; these bounded probes do not replace operator verification of the complete databases. The tool does not introduce cross-service tables or transactions; a partial failure can retain committed state in earlier databases, so the complete disposable run is retired rather than reset. Use test-only data-write credentials, not migration-administrator credentials. See the [tool guide](../../src/Tools/NexaConnect.PaymentReviewAcceptance/README.md).
