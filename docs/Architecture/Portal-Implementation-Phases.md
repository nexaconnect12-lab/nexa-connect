...
Payment intent creation remains closed. The authorization recovery slice adds explicit unknown outcomes, recoverable lease ownership/expiry, bounded attempts, provider status lookup, migration 4, Reporting migration 9, and Order `payment_pending` behavior that retains Inventory/Kitchen work for uncertain outcomes. Default automated coverage passes. Updated opt-in PostgreSQL authorization, 0→4→3→4 migration, Reporting replay, delayed/dropped provider, RabbitMQ, and forced-termination evidence remain release gates. Capture/void/refunds and established-dispatcher reconnection remain outside this slice.
