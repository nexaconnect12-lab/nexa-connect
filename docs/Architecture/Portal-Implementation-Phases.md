...
Phase 11 now includes opt-in live Catalog PostgreSQL and RabbitMQ acceptance plus Inventory PostgreSQL transactional acceptance. All five Inventory isolated-schema tests passed against local real PostgreSQL, covering outbox/audit atomic rollback, tenant isolation, competing-order concurrency, concurrent same-order stable identity, idempotent release, append-only audit, and the migration 5 downgrade/re-upgrade repository-script cycle. The Catalog RabbitMQ case verifies an unreachable connection attempt, a Catalog commit made without a broker connection, and later publication over a new real connection using production publisher confirms, persistent messages, an isolated queue, and publication timestamps. It neither stops a running broker nor proves automatic reconnection of an established dispatcher connection. No equivalent Inventory RabbitMQ recovery or full migration-runner 0→5 acceptance has completed; full service clean installs, broader service-owned PostgreSQL controller coverage, load/security validation, and release-environment browser execution remain continuous work.
