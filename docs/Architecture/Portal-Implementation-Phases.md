...
POS adds successful PostgreSQL evidence for cash replay and migration-4 manual-tender projection. The upgraded protected SQLite/PostgreSQL/RabbitMQ/migration runner passed 17/17 on 2026-09-10, followed by joined local SQLite-backed cash-checkout acceptance. Terminal-bound SQLite operational/outbox persistence now covers protected payloads, restart, fail-closed migration/corruption, scope mismatch, and queue transitions. Physical-terminal acceptance, server-side intermediate-workflow recovery, and broader operation synchronization remain open.
