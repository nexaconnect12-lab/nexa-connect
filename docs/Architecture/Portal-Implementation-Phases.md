...
POS adds nine API cases and seven successful isolated-schema PostgreSQL 17 cases for its cash-replay foothold. Live coverage proves exact/concurrent deduplication, rollback, terminal/shift-subject denial, active terminal scope, shift persistence/concurrency, and duplicate-open conflict. The actual migration runner passed 0→3→2→3 with checksum/history, schema transition, retained replay data, and repository verification. Native SQLite/replay tests and broader operation synchronization remain open.

Payment adds five successful local PostgreSQL 17/RabbitMQ acceptances plus a live Reporting migration-4 projection case. Live coverage proves atomic intent/audit/two-event commit/rollback, tenant-leading reads, concurrency/idempotency, append-only audit, confirmed recovery publication, mechanical legacy backfill, downgrade collision refusal, and the actual runner's 0→2→1→2 lifecycle; unit coverage proves exact Order-workload restriction and bounded financial/actor values. Production backfill still requires authoritative Order reconciliation. The Payment intent-creation Phase 10 slice is closed. Provider authorization, capture/refund workflows, reconciliation, and established-dispatcher reconnection remain outside this evidence.
