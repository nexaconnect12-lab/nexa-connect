...
The Bangkok manual-tender backend, POS projection, WPF cashier Paid flow, and single-store supervisor Cash Review slice are implemented through Order migration 6, Authorization migration 7, Reporting migration 14, and POS migration 5. Order migration 6 adds exact pre-payment stages and lease-fenced recovery of interrupted manual-tender Inventory/Kitchen work through KitchenAccepted. The backend/POS PostgreSQL and RabbitMQ manual-tender matrix passed 3/3, and the cashier joined flow has separate live evidence. Cash Review adds read/resolve permission separation, closed-session history, immutable version-fenced variance decisions, and terminal-bound SQLite schema-2 recovery of an uncertain supervisor request across restart. The guarded supervisor verifier is implemented; fresh joined physical-terminal execution and a live Order process-interruption recovery run remain release work.
...
Durable void/reversal recovery is implemented through Payment migration 7, Order migration 3, and Reporting migration 12. Payment enforces authorized-and-uncaptured eligibility, stable provider idempotency, bounded PostgreSQL status reconciliation, guarded downgrade, and transactional lifecycle/audit publication. Order's durable inbox protects paid orders, compensates confirmed voids, retains uncertain work, and records definitive failure/exhaustion as `payment_review`. Local PostgreSQL evidence passed Payment `0→7→6→7` and Order `0→3→2→3`. Live RabbitMQ void replay, dedicated telemetry/alerts, concrete-provider acceptance, refunds, and settlement remain open.

Order migration 4, Authorization migration 5, and Reporting migration 13 implement payment-review resolution: authenticated tenant-scoped routes, post-migration-safe resolver grants and accountant read-only access, fenced decision leases, confirm/resume/escalate behavior, append-only history with Authorization decision identity, transactional resolution/audit events, metrics, and a stale-review alert. The live Order runner passed `0→4→3→4`. The guarded matrix now exercises repository-produced contracts through hosted Order dispatcher restart plus hosted Reporting projection/deduplication; successful execution of the expanded matrix against each verified release target and the operator UI remain open.
...
POS adds successful PostgreSQL evidence for cash replay, migration-4 manual-tender projection, and migration-5 cash review state/history. The protected cashier matrix and joined local cash-checkout acceptance remain recorded. Terminal-bound SQLite covers protected cashier recovery/outbox state; the POS API and WPF now provide version-fenced cashier close plus online single-store supervisor history and decisions with Authorization-7 read/resolve separation. Order migration 6 provides bounded server-side recovery of interrupted manual-tender workflows. Fresh supervisor/physical-terminal acceptance, multi-store review/export, Reporting projection, provider-payment workflow recovery, a live Order process-interruption run, and broader synchronization remain open.
