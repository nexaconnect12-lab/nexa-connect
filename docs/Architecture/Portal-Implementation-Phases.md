...
Payment intent creation remains closed. The authorization development slice adds an exact Order-workload route, concurrency-controlled lease, provider I/O outside database transactions, sanitized results, Payment migration 3, Reporting migration 8, and Order enforcement that accepts only `authorized`. Default automated coverage passes. The updated opt-in PostgreSQL authorization, 0→3→1→3 migration, Reporting replay, and provider-environment cases remain release evidence to execute. Abandoned-lease recovery, capture/void/refunds, reconciliation, and established-dispatcher reconnection remain outside this slice.
