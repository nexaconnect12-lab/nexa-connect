...
POS adds nine API cases and seven successful isolated-schema PostgreSQL 17 cases for its cash-replay foothold. Live coverage proves exact/concurrent deduplication, rollback, terminal/shift-subject denial, active terminal scope, shift persistence/concurrency, and duplicate-open conflict. POS migration-runner 0→3→2→3, native SQLite/replay tests, and broader operation synchronization remain open.
