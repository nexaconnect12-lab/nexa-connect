...
POS adds successful PostgreSQL evidence for cash replay and migration-4 manual-tender projection. The upgraded protected SQLite/PostgreSQL/RabbitMQ/migration runner passed 17/17 on 2026-09-10, followed by joined local SQLite-backed cash-checkout acceptance. Terminal-bound SQLite covers protected recovery/outbox state, while the POS API and Shift & cash UI now provide cashier/terminal-scoped reconciliation and version-fenced close. Physical-terminal acceptance, reconciliation history/supervisor sign-off, server-side intermediate-workflow recovery, and broader operation synchronization remain open.
