...
POS adds successful PostgreSQL evidence for cash replay, migration-4 manual-tender projection, and migration-5 cash review state/history. The protected cashier matrix and joined local cash-checkout acceptance remain recorded. Terminal-bound SQLite covers protected cashier recovery/outbox state; the POS API and WPF now provide version-fenced cashier close plus online single-store supervisor history and decisions with Authorization-7 read/resolve separation. Order migration 6 provides bounded server-side recovery of interrupted manual-tender workflows, with passing real-host process-interruption evidence. Fresh supervisor/physical-terminal acceptance, multi-store review/export, Reporting projection, provider-payment workflow recovery, and broader synchronization remain open.
