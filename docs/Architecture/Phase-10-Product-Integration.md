...
| Inventory | Product-owned stock/reservation interfaces; branch authorization; PostgreSQL repository; durable inbox; versioned stock/reservation/release contracts; active-order idempotency; tenant-scoped transactional publication through the migration-1 outbox; append-only audit migration 5. Trusted unscoped paths remain legacy/non-publishing. | Add live PostgreSQL atomicity/concurrency/migration evidence and real RabbitMQ recovery acceptance before closing the slice |
