...
| Inventory | Product-owned stock/reservation interfaces; branch authorization; PostgreSQL repository; durable inbox; versioned stock/reservation/release contracts; persisted reservation identity and concurrent active-order idempotency; tenant-scoped transactional publication through the migration-1 outbox; append-only audit migration 5. All five opt-in isolated-schema tests passed against local real PostgreSQL, covering atomic rollback, concurrency, tenant isolation, release idempotency, append-only audit, and the 5→4→5 repository-script cycle. Trusted unscoped paths remain legacy/non-publishing. | Add real RabbitMQ outage/recovery acceptance and the full migration-runner 0→5 lifecycle before closing the slice; the five-test PostgreSQL result does not satisfy either gate |
...
Notification and Catalog are the first closed vertical slices in this phase. Catalog has transactional publication, live PostgreSQL atomicity/rollback/migration coverage, and real RabbitMQ acceptance that separates an unreachable connection attempt and broker-independent commit from later confirmed publication over a new connection. The acceptance does not stop a running broker or prove automatic reconnection of an established dispatcher connection. Full Catalog clean-install evidence remains a separate production migration release gate. The table deliberately leaves the overall phase marked partial because other product services retain exit work.
