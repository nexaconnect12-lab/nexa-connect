...
Pre-payment recovery is also Order-owned. Order migration 6 stores exact `submitted`, `inventory_reserved`, and `kitchen_accepted` stages plus the original payment method and correlation ID. A hosted worker atomically claims eligible manual-tender orders with `FOR UPDATE SKIP LOCKED`, an opaque claim ID, and a bounded lease. Inventory reservation and Kitchen ticket creation remain separate idempotent service calls keyed by Order identity; only the corresponding accepted transition and integration event commit under the live fencing claim. Dependency failure releases the claim for delayed retry, and process loss relies on lease expiry. The worker never confirms cash or PromptPay payment and stops at KitchenAccepted for cashier verification.
