...
Payment intent creation requires organization context for customer and trusted Order calls; only the exact Order workload bypasses customer authorization. Authorization recovery distinguishes and reconciles uncertainty. Immediate capture is a separate Order-only transition after authorization: `authorized → capturing → captured|failed|capture_unknown`. Provider calls occur outside database transactions; local start/result transitions append audit and outbox events atomically. Payment migration 5 and Reporting migration 10 own capture state/vocabulary. Order becomes paid only for `captured`, compensates definitive failure, and retains work for uncertainty. Default automated coverage passes; opt-in PostgreSQL, 0→5→1→5 migration, Reporting replay, provider, forced-termination, and capture-recovery evidence remain release gates. Void, refunds, and settlement remain planned.
