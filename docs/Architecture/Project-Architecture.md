...
An initial Product Owner Portal compatibility application is implemented in `src/Frontend/apps/product-owner-portal`; ADR-006 still assigns the durable portal and BFF to the future shared-platform repository. It authenticates only through `NexaConnect.PlatformAdminBff` and currently covers creation/status and read slices for control-plane management, audit viewing, support requests, and directory-owned summary counts. Organization/user lifecycle completion and support approval/revocation remain planned. It never embeds customer operations.
