...
POS cash replay has successful live PostgreSQL evidence for exact/concurrent replay, rollback, terminal/subject denial, active scope, and shift concurrency. Migration 4 adds the immutable manual-tender projection. The upgraded protected SQLite/PostgreSQL/RabbitMQ/migration runner passed 17/17 on 2026-09-10. The WPF client now has terminal-bound, schema-versioned SQLite operational/outbox persistence with protected payloads, explicit retry states, retained acknowledgements, interrupted-send recovery, integrity checks, and fail-closed legacy migration. Fresh interactive evidence against SQLite, physical-terminal acceptance, and broader order/shift/device synchronization remain open.
