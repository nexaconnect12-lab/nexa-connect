...
The WPF POS separates Checkout, Payment, Shift & cash, and Terminal & sync with terminal-bound SQLite recovery. Shift & cash reads a POS-owned PostgreSQL reconciliation using cashier subject and terminal predicates; expected cash is computed from opening cash plus signed movements. Close compares the reviewed concurrency version and resource scope in the same update that stores expected cash, counted cash, variance, closure time, and the next version. Terminal & sync exposes bounded unresolved-operation metadata without protected payloads. The guarded joined SQLite-backed local OIDC/WPF cash checkout passed on 2026-09-10; server-side intermediate-workflow recovery and reconciliation history/supervisor workflows remain open. See the [POS cashier acceptance guide](../Deployment/POS-Cashier-Acceptance.md).
...
POS cash replay has successful live PostgreSQL evidence for exact/concurrent replay, rollback, terminal/subject denial, active scope, and shift concurrency. Migration 4 adds the immutable manual-tender projection. The upgraded protected SQLite/PostgreSQL/RabbitMQ/migration runner passed 17/17 on 2026-09-10, followed by joined SQLite-backed local OIDC/WPF checkout acceptance. The WPF client now has terminal-bound SQLite recovery/outbox state plus cashier/terminal-scoped reconciliation and version-fenced cash close. Physical-terminal acceptance, reconciliation history/supervisor sign-off, server-side intermediate-workflow recovery, and broader order/shift/device synchronization remain open.
