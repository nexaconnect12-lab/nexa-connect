...
POS cash replay has successful live PostgreSQL evidence for exact/concurrent replay, rollback, terminal/subject denial, active scope, and shift concurrency. Migration 4 adds the immutable manual-tender projection. Migration 5 adds the review aggregate: a current projection plus immutable decision history committed under a session lock. Review reads derive `review_required` when the reviewed financial version no longer matches the cash session, including after a late settlement. Decisions retain the reviewer and Authorization decision, use an idempotency payload hash, and fence both versions. The WPF client exposes this online workflow for one configured store; its protected schema-2 state retains one pending decision identity, exact scope, reason, and versions across interruption. The read-only inspector exposes only a pending-row count. The live supervisor verifier is implemented, while fresh physical-terminal execution, offline authorization/execution, multi-store review, export/Reporting projection, server-side intermediate-workflow recovery, and broader synchronization remain open.
