...
The [five-scenario hosted Omise test-account gate passed on 2026-09-18](Evidence/Omise-Five-Scenario-Recovery-Acceptance.md), including pre-authorization token handoff. Payment implements opt-in signed test webhooks with migration-8 durable event-ID ingestion, authenticated canonical event validation and lease-fenced status-only recovery. The guarded real external-delivery/process-interruption gate [passed on 2026-09-22](Evidence/Omise-Webhook-Recovery-Acceptance.md), including post-commit/pre-acknowledgement termination, expired-lease restart and signed duplicate replay. Production enablement remains open. [ADR-010](Decisions/ADR-010-verified-omise-webhook-recovery.md) records the trade-offs. See [configuration, failure recovery and live acceptance](../Deployment/Omise-Webhooks.md).

A read-only POS Omise acceptance verifier now checks one Paid test-card Order, one amount/ownership-matched captured intent with single authorization/capture starts, an operator-supplied closed shift and cleared local credentials/recovery. UI/OIDC steps require explicit operator confirmation; the verifier does not certify remote charge status or process interruption. [Local POS Omise verification passed on 2026-09-18](Evidence/Omise-POS-Card-Acceptance.md) with operator-attested UI/OIDC/sign-out; those interactions are not independently inspected. See the [card-checkout verification procedure](../Deployment/Omise-POS-Card-Token-Handoff.md).


Opt-in Development-only `card_omise_test` checkout now forwards a masked, transient test token from POS to Order and Payment. Pending intents without a token await a fresh operator token; retries use the original Order identity and contents. For an original pending checkout, POS enables token input only after Verify original order returns validated `cardTokenRequired=true`; that permission is memory-only and cleared on each attempt/session lock. Tokens never enter SQLite/outbox or durable workflow state. All card completion paths—foreground, workflow recovery and authorization/capture reconciliation—use a shared Order-owned deterministic event identity, allowing the transactional outbox to suppress concurrent completion re-enqueue; other payment methods retain their existing event identity behavior. The opt-in five-scenario hosted Omise gate passed on 2026-09-18, including pre-authorization/token handoff. Local POS Omise verification passed on 2026-09-18 with operator-attested UI/OIDC/sign-out; the verifier does not independently inspect those interactions. Production browser card collection remains planned. [ADR-009](Decisions/ADR-009-ephemeral-test-card-token-handoff.md) records transient-token and explicit-recovery trade-offs. See the [configuration and acceptance guide](../Deployment/Omise-POS-Card-Token-Handoff.md).

The hosted provider recovery runner now supports explicit Omise selection alongside unchanged GenericHttp defaults. Four independently tokenized response-boundary fixtures verify hosted recovery, broker outage, PostgreSQL outbox persistence and restart delivery through the existing service boundaries. The historical/default four-scenario Omise matrix excludes pre-authorization; its subsequent opt-in five-scenario test-token gate passed on 2026-09-18. No deployed API, schema or ownership changes; normal TLS and startup lease guards apply. Credentialed four-scenario hosted Omise acceptance passed on 2026-09-17; pre-authorization interruption and production acceptance remain outside this evidence. See [topology, guards and verification](../Deployment/Omise-Hosted-Recovery-Acceptance.md).
...
Omise capture diagnostics now include `scripts/inspect-omise-test-charge.ps1`: hidden input selects an existing test charge, a single read-only GET reports nullable flags/minor-unit amounts without identifiers, and no financial or local-state mutation occurs. The Paid-charge inspection identified omitted captured_amount. New authorizations retain signed full-capture request context, allowing strictly verified paid responses that omit both capture fields while preserving unknown raw values and partial/refund protection. See [ADR-008](Decisions/ADR-008-omise-full-capture-context.md) for request provenance and credential-rotation trade-offs. Existing unmarked charges remain unconfirmed; the direct Omise test-account gate passed on 2026-09-17, including trusted full-capture context; four-scenario hosted Omise acceptance passed on 2026-09-17; the subsequent pre-authorization/token-handoff gate passed on 2026-09-18; see the [Omise acceptance guide](../Deployment/Omise-Test-Account-Acceptance.md#inspect-an-existing-charge-without-another-payment).

A Development-only local HTTPS Omise token helper is available at `src/Tools/NexaConnect.OmiseTokenHelper`. It uses official browser tokenization with the operator's test public key and a fixed test card, keeps tokens out of server requests and logs, and replaces the documentation demo for acceptance preparation. Trusted development HTTPS is required; direct credentialed Omise acceptance passed on 2026-09-17; Development-only POS/Order test-token handoff is implemented; local POS verification passed on 2026-09-18 with operator-attested UI/OIDC; independent proof of UI interaction and production card collection remain outside this pass. See the [Omise acceptance guide](../Deployment/Omise-Test-Account-Acceptance.md).

Payment integrates an Omise anti-corruption adapter for test-account THB card authorization/capture/reversal/status. An optional internal authorize body supplies an ephemeral test card token; pending-intent input is validated before a lease and is never persisted. Charge responses and recovery search are matched to Payment/organization/Order metadata and amount/currency/test mode. Omise POSTs have no automatic retry; bounded existing recovery performs lookup only. Selection is restricted to Development/Testing and official HTTPS with test keys. No schema or service ownership changes. The direct sandbox runner remains credential-gated; production card collection/independent proof of UI interaction, 3DS, PromptPay and production onboarding are not implemented. See [contract and operations](../Deployment/Omise-Test-Account-Acceptance.md).
...
Provider recovery acceptance provisions a durable evidence queue bound to `payment.#` on its generated broker before starting Payment. The queue supplies the lifecycle/audit subscription required by mandatory publisher confirmation, independently of Order's reconciliation subscriptions. A generated-project named RabbitMQ volume retains queue/binding state across stop/start and one randomly selected loopback port remains fixed; project cleanup removes the volume. The harness receives and verifies the exact terminal event with persistent delivery after restart while separately checking Order inbox completion. This is acceptance topology only and leaves production transport confirmations and product boundaries unchanged.

The acceptance-only `NexaConnect.PaymentProviderSimulator` provides in-memory authorization/capture/void/status over generated loopback HTTPS. The local recovery wrapper generates a short-lived certificate and credential, checks replay and credential rejection, then runs real Order/Payment hosts with disposable PostgreSQL/RabbitMQ. It never changes Windows trust stores. Payment's optional exact SHA256 simulator certificate pin is rejected outside Testing or outside `https://127.0.0.1`, and hostname errors remain rejected. Ordinary provider clients retain platform TLS validation. This fixture introduces no product data ownership, API or persistence schema change; it cannot certify an external provider. The complete local hosted matrix passed on 2026-09-16 with three provider-boundary interruptions, three Payment-host restarts and cleanup; four-scenario hosted Omise execution subsequently passed on 2026-09-17; five-scenario Omise pre-authorization acceptance passed on 2026-09-18; production acceptance remains a release gate. See [simulator setup and limitations](../../src/Tools/NexaConnect.PaymentProviderSimulator/README.md).
...
The WPF POS separates Checkout, Payment, Shift & cash, Cash review, and Terminal & sync with terminal-bound SQLite recovery. Shift & cash reads a POS-owned PostgreSQL reconciliation using cashier subject and terminal predicates and closes against its reviewed concurrency version. Cash review queries an exact configured store, separates read and resolve permissions, and records idempotent investigating/approval decisions against financial and review versions. SQLite schema 2 persists the exact supervisor request before network send and restores it as a locked verification action after restart. POS migration 5 owns current review state and append-only history; Authorization migration 7 owns role grants. The guarded cashier checkout passed on 2026-09-10, and the guarded local human Cash Review workflow passed on 2026-09-15. Order migrations 6-7 own server-side recovery of interrupted manual and provider-payment workflows. The guarded provider runner covers intent-created, authorization-response, and capture-response boundaries with generated PostgreSQL/RabbitMQ and real Order and Payment hosts. It exercises hosted Payment recovery during broker outage, requires the terminal state and unpublished outbox event to persist before Payment is killed, then verifies publication and matching Order inbox completion after RabbitMQ and Payment restart. The harness builds cleanly; local hosted-simulator acceptance passed on 2026-09-16; credentialed external-sandbox evidence remains a release gate. Fresh physical-terminal session-lock and supervisor acceptance, multi-store views, exports and production cash-close Reporting acceptance, offline supervisor authorization/execution, and broader synchronization remain open. See the [POS cashier acceptance guide](../Deployment/POS-Cashier-Acceptance.md).

The POS settlement consumer exposes correlated applied/replayed/retry/dead-letter telemetry. A guarded acceptance runner targets the migration lifecycle, store projection, and hosted durable-queue restart/replay/dead-letter behavior. Prometheus monitors queue depth, dead letters, and retries. The disposable matrix passed 3/3 on 2026-09-03 UTC, including the actual `0→4→3→4` migration lifecycle; production monitoring and paging validation remain release work.

Manual tender settlement is Order-owned, not provider capture. Order migration 5 commits Paid state, settlement, `order.manual-tender-settled.v1`, and audit. POS migration 4 consumes through a durable queue and immutable Order-unique projection. Cash is attributed by the event-time shift/THB-session window; late delivery reconciles the historical closed-session variance rather than charging the current drawer. PromptPay never changes cash totals. No transaction crosses service databases.

Checkout recovery is also Order-owned. Order migration 6 stores exact `submitted`, `inventory_reserved`, and `kitchen_accepted` stages plus the original payment method and correlation ID. Migration 7 expands the recovery index to all persisted checkout methods in Submitted/InventoryReserved and provider methods in KitchenAccepted. A hosted worker atomically claims eligible orders with `FOR UPDATE SKIP LOCKED`, an opaque claim ID, and a bounded lease. Inventory reservation and Kitchen ticket creation remain separate idempotent service calls keyed by Order identity; only the corresponding accepted transition and integration event commit under the live fencing claim. Provider recovery creates or reloads the stable `order:{orderId}` Payment intent. Captured intents complete, authorized intents resume capture, pending intents begin authorization, and in-progress or uncertain states are never replayed. Those states bind the original intent to a PaymentPending Order. When authorization recovery later reports `authorized`, the durable Order consumer resumes capture through the state-aware adapter; in-progress capture then belongs to Payment's capture recovery. Exhausted authorization or capture enters PaymentReview. Definitive payment failure runs idempotent Inventory/Kitchen compensation before the terminal Order/outbox transaction. Dependency failure releases the claim for delayed retry, and process loss relies on lease expiry. Cash and PromptPay still stop at KitchenAccepted for cashier verification.
...
POS cash replay has successful live PostgreSQL evidence for exact/concurrent replay, rollback, terminal/subject denial, active scope, and shift concurrency. Migration 4 adds the immutable manual-tender projection. Migration 5 adds the review aggregate: a current projection plus immutable decision history committed under a session lock. A late cash projection updates stored expected closing cash, variance, and financial version in its settlement transaction. Review reads independently derive expected cash from authoritative movements, including for legacy closed rows with a missing or stale stored expected amount, and derive `review_required` when the reviewed financial version no longer matches. Decisions retain the reviewer and Authorization decision, use an idempotency payload hash, and fence both versions. The WPF client exposes this online workflow for one configured store; its protected schema-2 state retains one pending decision identity, exact scope, reason, and versions across interruption. The read-only inspector exposes only a pending-row count. The guarded local human supervisor verifier passed on 2026-09-15 with three decisions over two financial versions and clean schema-2 recovery state. Order's guarded manual-tender real-host recovery acceptance passed on 2026-09-15 with two process interruptions, a dependency retry cycle, original correlation propagation, exact transition counts, final settlement, and project cleanup. Provider-payment recovery and its five-scenario hosted Payment/outbox harness are implemented; local hosted-simulator acceptance passed on 2026-09-16 and the credentialed Omise test-account matrix passed through pre-authorization token handoff on 2026-09-18. Production provider acceptance remains a release gate. Fresh physical-terminal session-lock and supervisor execution, offline authorization/execution, multi-store review, export and production cash-close Reporting acceptance, and broader synchronization remain open. See [Order recovery evidence](Evidence/Phase-10-Order-Workflow-Recovery.md).
...
Capture recovery is implemented through provider status lookup, recoverable capture leases, a bounded recovery worker, conservative resolution of `capturing`/`capture_unknown`, transactional `PaymentCaptureReconciledV1` publication, durable Order reconciliation, Payment migration 6, Order migration 2, and Reporting migration 11. Void recovery and Order consumption extend the same conservative pattern through Payment 7, Order 3, and Reporting 12. Provider selection is explicit at composition: `Disabled` is the fail-closed default, returns bounded `provider_not_configured` command failures, leaves status unknown, and prevents recovery-worker registration so durable uncertainty is not consumed before onboarding; `GenericHttp` activates the existing adapter. `PaymentProvider:CaptureRecoveryEnabled` and `PaymentProvider:VoidRecoveryEnabled` then pause their respective workers without mutating durable state. The shared RabbitMQ transport owns a lazy recoverable connection while durable rows remain retryable. The HTTP provider adapter requires HTTPS, accepts a secret-injected bearer credential, uses bounded request timeouts and stable operation-specific idempotency keys, and maps timeouts, transport errors, HTTP 408/429/5xx, or malformed success bodies to reconciliation-required uncertainty without retaining provider payloads. A provider-neutral, opt-in live-sandbox acceptance uses distinct disposable capture/void authorizations to verify a real TLS handshake, configured-credential acceptance, replay identity, and status reconciliation. A separate guarded Order recovery runner creates five new sandbox authorizations, three captures and two pre-capture voids, interrupts the server-side process at intent-created and post-response boundaries, and verifies exact durable command-start counts plus final Payment/Order state through generated PostgreSQL/RabbitMQ. It starts real Order and Payment hosts, stops RabbitMQ before hosted Payment recovery, requires an unpublished terminal outbox message with a failed publish attempt, kills Payment after that commit, and verifies publication plus matching Order inbox completion after broker and Payment restart. Successful JSON evidence is identifier-free; failed child output remains restricted diagnostics. The complete local hosted matrix passed on 2026-09-16, including restart-persistent broker topology, exact terminal-event delivery and cleanup; four-scenario Omise hosted execution passed on 2026-09-17; five-scenario Omise pre-authorization acceptance passed on 2026-09-18; production acceptance remains a release gate. Invalid-credential rejection, provider-specific credential rotation, quota/rate-limit and lost-response fault evidence, production paging/escalation, partial-compensation process-loss acceptance, dedicated void alerts, refunds, and settlement remain release or subsequent-slice work.

Manual tender settlement is Order-owned rather than provider capture. Order 5 owns immutable settlement/outbox, Authorization 6 owns grants, Reporting 14 owns audit vocabulary, and POS 4 consumes the event idempotently with event-time cash-session attribution. The WPF cashier client uses `cash_manual`/`promptpay_manual` checkout to stop at KitchenAccepted, then submits the exact Order-owned settlement with explicit operator confirmation. It retains the idempotency identity and exact attempted tender fields locally for safe verification after an uncertain response or restart. Joined SQLite-backed local OIDC/WPF cash checkout, Order settlement, POS projection, cash close, shift close, sign-out, and cleared local recovery passed on 2026-09-10; production deployment and physical-terminal verification remain separate gates.
...
The single-store cash-close Reporting slice adds POS migration-6 publication checkpoints and Reporting migration-15 facts/receipt hashes. An opt-in POS scanner resolves Restaurant-owned organization scope, then locks the session and commits a full snapshot/outbox event with its monotonic checkpoint; it backfills closed sessions and coalesces intermediate changes. Reporting's separate opt-in consumer commits receipt and newest fact together before acknowledgement, rejecting ownership/version conflicts and stale replacement. Restricted financial snapshots remain separate from the safe activity feed. BFF membership checks precede Reporting reads; Reporting calls POS for live exact-store `pos.cash-review.read`, never POS tables. Bounded UTC pages expose capture/projection times without totals or completeness guarantees. POS downgrade is guarded after publication; destructive Reporting rollback needs retained-event replay after re-upgrade. POS migration 7 adds append-only replay runs/attempts. An operational CLI pins exact retained events to a preview manifest, preserves original identities and requires independent binding/projection checks; broker confirmation is not consumer completion. A real-process recovery runner and backlog/retry alerts are implemented, and local Windows recovery/restricted replay passed 2/2 with no skips on 2026-09-24, alongside six-rule Prometheus evaluation. An Ubuntu PR/manual CI gate is authored but unexecuted remotely; branch protection, production acceptance and receiver delivery remain open. See [evidence](Evidence/Cash-Close-Recovery-Acceptance.md). See [recovery runbook](../Deployment/Cash-Close-Recovery.md). See [contract and deployment](../API/Cash-Close-Reporting.md) and [ADR-012](Decisions/ADR-012-cash-close-snapshot-reporting.md).
...
The implemented offline synchronization foothold is cash-movement replay idempotency. The native POS client commits stable operation/terminal identity and protected payloads to SQLite before its first HTTP attempt and sends that same identity on every replay. Queue transitions are atomic and explicit (`queued`, `sending`, `rejected`, `completed`); startup returns interrupted sends to `queued`, permanent `400`/`403`/`409` responses require authenticated retry, and successful acknowledgements remain locally retained until a later retention purge. POS Infrastructure verifies terminal and subject against the shift, then commits its terminal-scoped dedupe marker with the cash movement. Exact/concurrent retries do not duplicate totals, transaction failure rolls back the marker, scope mismatch fails, and mismatched payload reuse conflicts. Cash close remains blocked by unresolved movement rows. Full offline order/shift/device synchronization remains open.
...
Payment Review's operations acceptance owns a generated `nexa-review-it-<id>` Compose project with ephemeral PostgreSQL/RabbitMQ and isolated alert delivery; all 13 matrix cases passed locally. The joined launcher owns PostgreSQL, Keycloak, RabbitMQ, Toxiproxy, seven loopback application hosts, and eight fresh fixtures in one disposable lifecycle. It validates tenant ownership, routes Order-to-Inventory through the proxy, and exposes a generated-token loopback controller that can stop/restart only the disposable Inventory and Kitchen children. All ten browser scenarios and cleanup passed locally on 2026-09-02, including individual and combined process loss. Production persistence/orchestrator recovery, provider evidence, paging, and live-traffic rollback remain gates. No production topology, financial transition, or service-owned schema changed.
...
The [joined cash-close portal harness](../Deployment/Cash-Close-Portal-Acceptance.md) owns a disposable Keycloak/PostgreSQL/RabbitMQ project and six loopback hosts (Platform Directory, Authorization, Restaurant, POS, Reporting and Customer BFF). A test-process-owned TCP proxy interrupts Reporting-to-POS access; no production fault endpoint is added. Fixtures use service-owned provisioning and POS repositories, with store bootstrap and read-deny changes isolated in fixture Infrastructure. Five real-OIDC browser cases exercise eventual propagation and fail-closed reads. Local execution evidence is pending; the separate Ubuntu CI job requires all five cases plus cleanup and uploads only bounded JSON summaries.
