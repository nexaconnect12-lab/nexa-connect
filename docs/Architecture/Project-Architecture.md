...
Its first authenticated product adapters are Catalog menu, Inventory stock, and the Order place-workflow. They forward the server-held bearer token and validated tenant headers; the BFF derives organization and branch IDs from the protected tenant selection and route rather than trusting duplicate browser payload fields. Catalog verifies organization access through Platform Directory, then uses a Catalog workload identity to read the Restaurant-owned branch scope. Order independently repeats organization and branch authorization before running its workflow. Product-specific permission evaluation remains owned by each product boundary.
