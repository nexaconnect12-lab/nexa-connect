...
Payment intent creation requires organization context for customer and trusted Order calls; only the exact Order workload bypasses customer authorization. The development authorization slice adds an Order-only operation, a concurrency-controlled `authorizing` lease committed before provider I/O, and a separate terminal-result transaction with sanitized provider reference or bounded failure category. Each transition appends local audit plus lifecycle and audit outbox events; Reporting migration 8 accepts the authorization audit vocabulary. Order now requires `authorized` and compensates Inventory/Kitchen otherwise. Default automated coverage passes; opt-in PostgreSQL authorization, 0→3→1→3 migration, Reporting replay, and provider-environment evidence remain release gates. Capture, void, refunds, abandoned-lease recovery, and reconciliation remain planned.
