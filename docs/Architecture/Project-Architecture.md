...
Inventory transactionally records tenant-scoped stock-set, reservation-created, and reservation-released contracts with append-only audit state. Migration 1 owns its outbox and migration 5 owns only product audit. Trusted unscoped PostgreSQL paths remain legacy/non-publishing. Active-order reservation retries are idempotent; live PostgreSQL concurrency/migration and RabbitMQ recovery evidence remain open.
