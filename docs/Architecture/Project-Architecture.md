...
Manual tender settlement is Order-owned, not provider capture. Order migration 5 and the branch-authorized route accept only exact-total THB `cash` or receipt-confirmed `promptpay_manual` for eligible, non-provider-bound Orders. PostgreSQL atomically commits Paid state, an immutable settlement, `order.manual-tender-settled.v1`, and safe audit; unique Order and tenant/branch/idempotency constraints plus request fingerprints prevent double settlement and ambiguous replay. Authorization migration 6 grants cashier/store-manager/tenant-admin confirmation and Reporting migration 14 accepts the audit action. The event is the future idempotent POS cash-session boundary; POS consumption must not introduce a distributed transaction.
...
Manual tender settlement is an Order-owned capability rather than provider capture. Order migration 5 operationalizes exact-total THB `cash` and receipt-confirmed `promptpay_manual` with ownership/state/provider guards, immutable persistence, and atomic settlement/audit/outbox commit. Authorization migration 6 owns confirmation grants and Reporting migration 14 owns the audit vocabulary. POS cash-session event consumption and the cashier Paid control remain next.
