...
Its first authenticated product adapter is the Catalog menu adapter. It forwards the server-held bearer token and validated tenant headers; Catalog verifies organization access through Platform Directory, then uses a Catalog workload identity to read the Restaurant-owned branch scope. A branch whose scope organization does not match the selected organization is rejected before menu data is read; product-specific permission evaluation remains owned by the Catalog/product authorization boundary.
