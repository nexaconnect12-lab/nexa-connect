...
POS cash-movement replay now has successful live PostgreSQL evidence. Seven isolated-schema cases passed locally against PostgreSQL 17, covering exact and concurrent replay, atomic rollback, terminal/shift-subject denial, active terminal scope, shift persistence/concurrency, and duplicate-open conflict. The run exposed and corrected an Npgsql multi-command completion issue before acceptance. POS 0→3→2→3 migration-runner evidence, production SQLite client persistence, and order/shift/device synchronization remain open.
