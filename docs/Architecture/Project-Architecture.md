...
Payment intent creation requires organization context for customer and trusted Order calls; only the exact Order workload bypasses customer authorization. Authorization distinguishes declined, failed, and unknown outcomes, commits a recoverable `authorizing` lease before provider I/O, and records sanitized terminal or reconciliation results in separate transactions. Payment migrations 3-4 and Reporting migrations 8-9 accept the lifecycle/reconciliation audit vocabulary. Order marks uncertain provider outcomes `payment_pending`, retaining Inventory/Kitchen work until reconciliation authoritatively authorizes or fails. Default automated coverage passes; opt-in PostgreSQL authorization, 0→4→3→4 migration, Reporting replay, and provider-environment evidence remain release gates. Capture, void, refunds, and financial settlement remain planned.
