...
Manual tender settlement is Order-owned rather than provider capture. Order 5 owns immutable settlement/outbox, Authorization 6 owns grants, Reporting 14 owns audit vocabulary, and POS 4 consumes the event idempotently with event-time cash-session attribution. The cashier Paid control remains next.
