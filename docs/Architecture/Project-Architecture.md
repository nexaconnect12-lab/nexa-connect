...
Manual tender settlement is being introduced as an Order-owned capability rather than pretending cash or visually checked PromptPay is a remote provider capture. The domain foundation accepts only `cash` and `promptpay_manual`, exact Order-total THB, matching organization/branch, an eligible accepted or payment-pending state, and non-empty terminal/operator/idempotency identities. PromptPay requires an explicit receipt-confirmed assertion; provider-bound Orders are excluded to prevent double settlement. The foundation is not yet an operational capability: durable settlement/audit/outbox state, Authorization grants, POS cash-session coordination, Reporting vocabulary, HTTP/BFF routes, and the cashier Paid control remain the next atomic slice.
