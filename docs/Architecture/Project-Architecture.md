...
The Phase 8 React context/navigation shell is independently built from `src/Frontend/apps/customer-portal` and published with the Customer BFF. Client pages are gated by current organization/application access; `/tenant` and `/features/*` revalidate the protected pair server-side. Profile and product switching use real access data, dashboards are context-only, and other product-owned feature UI remains staged behind `contract-pending` availability responses.
