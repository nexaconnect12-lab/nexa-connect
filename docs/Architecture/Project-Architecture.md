...
Capture recovery is implemented through provider status lookup, recoverable capture leases, a bounded recovery worker, conservative resolution of `capturing`/`capture_unknown`, transactional `PaymentCaptureReconciledV1` publication, durable Order reconciliation, Payment migration 6, Order migration 2, and Reporting migration 11. `PaymentProvider:CaptureRecoveryEnabled` is an operational registration switch that pauses capture recovery without changing durable state or disabling authorization recovery. The shared RabbitMQ outbox transport owns a lazy recoverable connection and invalidates/replaces it after closure or publish failure while durable rows remain unpublished for retry. Payment/Order backlog-age gauges and RabbitMQ queue/dead-letter metrics feed checked-in local alerts. The safety-gated release script repeats the disposable acceptance matrix but deliberately does not attest a provider-connected OS process kill or external alert delivery. Local PostgreSQL, RabbitMQ, rollback, Reporting replay, migration, deterministic response-loss, and established-client replacement evidence passed; the two external exercises remain release gates. Void/reversal, refunds, compensation-policy expansion, and end-of-day settlement remain subsequent slices.
