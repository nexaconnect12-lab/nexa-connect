...
The implemented offline synchronization foothold is cash-movement replay idempotency. Native POS clients may send `X-Client-Operation-Id` when retrying local outbox movements; POS Infrastructure stores that terminal-scoped operation in `sync_operations` and commits it with the cash movement so exact retries are accepted without duplicating cash totals, while mismatched payload reuse fails with `409`.
