...
The Phase 8 React portal is independently built and published with the Customer BFF. Profile, product switching, memberships, branches, typed configuration, dashboards, sales, activity preview, and Media list and preview upload/download/delete use real contracts. Media owns metadata/object keys, validates Catalog ownership through an endpoint-specific Media workload policy, and issues short-lived S3-compatible URLs so bytes do not traverse the BFF. It verifies provider-returned size and SHA-256 before readiness; malware scanning, quotas, variants, and cross-store reconciliation remain staged.
