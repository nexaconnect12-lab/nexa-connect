...
Capture recovery is implemented through provider status lookup, recoverable capture leases, a bounded recovery worker, conservative resolution of `capturing`/`capture_unknown`, transactional `PaymentCaptureReconciledV1` publication, durable Order reconciliation, Payment migration 6, Order migration 2, and Reporting migration 11. `PaymentProvider:CaptureRecoveryEnabled` is an operational registration switch that pauses capture recovery without changing durable state or disabling authorization recovery. The safety-gated release script repeats the disposable acceptance matrix but deliberately does not attest provider process-kill, established-connection restart, or alert delivery. Local PostgreSQL, RabbitMQ, rollback, Reporting replay, and migration evidence passed on 2026-08-26; those three environment-specific checks remain release gates. Void/reversal, refunds, compensation-policy expansion, and end-of-day settlement remain subsequent slices.
