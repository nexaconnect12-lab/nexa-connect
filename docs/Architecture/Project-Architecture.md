...
Capture recovery is implemented through provider status lookup, recoverable capture leases, a bounded recovery worker, conservative resolution of `capturing`/`capture_unknown`, transactional `PaymentCaptureReconciledV1` publication, durable Order reconciliation, Payment migration 6, Order migration 2, and Reporting migration 11. Local PostgreSQL, RabbitMQ, rollback, Reporting replay, and migration evidence passed on 2026-08-26; production-provider process termination, established broker-connection recovery, and alert delivery remain release gates. Void/reversal, refunds, compensation-policy expansion, and end-of-day settlement remain subsequent slices.
