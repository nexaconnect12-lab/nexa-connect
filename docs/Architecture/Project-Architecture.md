...
Payment intent creation requires organization context for customer and trusted Order calls; only the exact Order workload bypasses customer authorization. Authorization and capture recovery distinguish definitive outcomes from uncertainty. Immediate capture is a separate Order-only transition after authorization: `authorized → capturing → captured|failed|capture_unknown`. Provider calls and capture-status lookups occur outside database transactions; local transitions append audit and outbox events atomically. Payment migration 6 owns capture recovery leases/attempts and Reporting migration 11 owns reconciliation vocabulary. Order migration 2 durably consumes `PaymentCaptureReconciledV1`; authorization alone cannot complete an order, only `captured` marks it paid, definitive failure compensates, and unknown/exhausted results retain work. Sixteen coordinated cases passed locally on 2026-08-28. Gated harnesses additionally passed child-process recovery, full RabbitMQ restart, isolated firing/resolved webhook delivery, and disposable Payment/Order rollback-forward lifecycles. Concrete-provider, production paging/escalation, and live-traffic rollback validation remain environment-specific gates. Void, refunds, and settlement remain planned.
...
The Payment lifecycle uses a small Domain aggregate for validation and normalization, Application-owned repository/provider ports, and Infrastructure-owned PostgreSQL/outbox/provider adapters. Migration 2 adds organization attribution and append-only audit while migration 1 remains the outbox owner; migrations 3-4 add recoverable authorization, migration 5 adds immediate capture, and migration 6 adds capture-specific recovery state. Matching idempotent intent retries return the original without republishing. Capture is Order-only, uses separate local transactions around provider I/O, and sends the intent ID as the provider idempotency key. Recovery performs status lookup rather than repeating capture and publishes a versioned reconciliation event for Order. The initial application-level void boundary is also Order-only and accepts only authorized, uncaptured intents; it uses `void:{intent-id}` for provider idempotency, distinguishes failure from uncertainty, and treats captured payments as refund-only. PostgreSQL void recovery/events, refunds, and settlement are not enabled.

Capture recovery is implemented through provider status lookup, recoverable capture leases, a bounded recovery worker, conservative resolution of `capturing`/`capture_unknown`, transactional `PaymentCaptureReconciledV1` publication, durable Order reconciliation, Payment migration 6, Order migration 2, and Reporting migration 11. `PaymentProvider:CaptureRecoveryEnabled` pauses capture recovery without changing durable state or disabling authorization recovery. The shared RabbitMQ transport owns a lazy recoverable connection while durable rows remain retryable. Local gated harnesses verify process/broker faults, synthetic Alertmanager firing/resolution delivery, and generated-database downgrade/re-upgrade with guards. The alert profile is isolated from the default observation-only receiver and all successful-run resources are removed. Concrete-provider credentials/TLS/rate limits, production paging/escalation, and live traffic draining remain release gates. PostgreSQL void recovery, refunds, compensation-policy expansion, and end-of-day settlement remain subsequent slices.

Retained local evidence is recorded in [Phase 11 capture recovery](Evidence/Phase-11-Payment-Capture-Recovery.md) and [Phase 12 operational rehearsal](Evidence/Phase-12-Payment-Operational-Rehearsal.md).
