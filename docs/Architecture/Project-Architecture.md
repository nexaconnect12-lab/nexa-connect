...
The Phase 8 React portal is independently built from `src/Frontend/apps/customer-portal` and published with the Customer BFF. Profile, product switching, memberships, branches, typed branch configuration, dashboards, sales reports, and media metadata use real contracts. Restaurant owns configuration and transactional audit; Reporting reads only its rebuildable projections and exposes freshness; Media owns metadata and the future object lifecycle. Each service independently checks Platform Directory access and operation-specific Authorization permissions. Unified activity and Media object workflows remain staged.
