...
Payment Review's operations acceptance owns a generated `nexa-review-it-<id>` Compose project with ephemeral PostgreSQL/RabbitMQ and isolated alert delivery. Its launcher injects generated settings instead of application credentials and verifies scoped cleanup; all 13 matrix cases passed locally. The separate seven-scenario live browser harness requires an operator-provisioned disposable Keycloak realm, actual BFF/Order/dependency services, five fresh fixtures, and a run-scoped loopback Inventory fault proxy; it validates fixture markers and tenant ownership and keeps credentials out of evidence. The proxy-outage case requires the failed attempt to leave no committed history and restoration to require a fresh explicit decision. It is implemented but not live-executed here; Inventory process loss, Kitchen/combined outage and provider evidence remain separate gates. See [release checklist](../Deployment/Payment-Review-Release-Checklist.md). No production topology, financial transition or service-owned schema changed.

The test-only `NexaConnect.PaymentReviewAcceptance` tool provisions the joined fixture graph through service-owned Platform Directory, Restaurant, Authorization, and Order boundaries and emits only synthetic identifiers. Exact generated database-name checks and bounded service-owned probes reject existing fixture root/state data in all four databases before the first write; operators still verify the complete databases are fresh, and the tool is not a reset path. Cross-service atomicity is not claimed: any partial failure requires retirement of the entire disposable run. Identity, process, proxy, and lifecycle orchestration remain outside the tool.
