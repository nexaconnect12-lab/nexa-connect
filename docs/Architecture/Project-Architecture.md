...
The [five-scenario hosted Omise test-account gate passed on 2026-09-18](Evidence/Omise-Five-Scenario-Recovery-Acceptance.md), including pre-authorization token handoff. Payment now implements opt-in signed test webhooks with migration-8 durable event-ID ingestion, authenticated canonical event validation and existing lease-fenced status-only recovery. Notifications supplement polling; they do not issue financial commands or replace current-charge validation. External signed delivery acceptance and production enablement remain outside this pass. [ADR-010](Decisions/ADR-010-verified-omise-webhook-recovery.md) records authentication and status-only recovery trade-offs. See [configuration, failure recovery and telemetry](../Deployment/Omise-Webhooks.md).
