...
The Phase 8 React portal is independently built from `src/Frontend/apps/customer-portal` and published with the Customer BFF. Profile, product switching, and membership management use real contracts. Membership routes derive the organization from the protected tenant; Platform Directory independently enforces customer owner/admin roles and active access, rejects self-mutation, applies optimistic concurrency, and writes the membership and audit record transactionally. Other operational UI remains staged.
