...
The Phase 8 React portal is independently built from `src/Frontend/apps/customer-portal` and published with the Customer BFF. Profile, product switching, memberships, branches, typed branch configuration, dashboards, sales reports, activity-projection preview, and media metadata use real contracts. Restaurant owns configuration and transactional audit; Reporting owns rebuildable metric and activity projections; Media owns metadata and the future object lifecycle. Activity ingestion accepts the versioned safe audit contract idempotently behind an allow-listed workload boundary; source publication and RabbitMQ consumption remain staged.
