...
The WPF POS separates Checkout, Payment, Shift & cash, and Terminal & sync. Terminal-bound SQLite protects operational/outbox payloads and exact retry identity. POS-owned PostgreSQL now supplies the cashier/terminal-scoped cash reconciliation: opening cash, signed movements, expected cash, counted variance, history, and concurrency version. Cash close is fenced by the reviewed version and stores the financial result atomically. The upgraded protected SQLite/backend runner and joined SQLite-backed OIDC/WPF cash checkout passed on 2026-09-10. Server-side intermediate-workflow recovery and multi-session reconciliation/supervisor workflows remain open. See the [POS cashier acceptance guide](../Deployment/POS-Cashier-Acceptance.md).
