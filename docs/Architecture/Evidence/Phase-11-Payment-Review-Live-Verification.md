...
The expanded joined slice passed 10/10 locally on 2026-09-02 in generated project `nexa-review-joined-0974977248ee4f3bb45b24c91fe7c881`. Its sanitized summary records every migration, identity, fixture, application, proxy, process-cleanup, Compose-cleanup, and live-browser flag as true. The run used PostgreSQL, Keycloak, RabbitMQ, Toxiproxy, and seven application hosts; authenticated distinct reader/resolver sessions; and exercised concurrency, resume, void, Inventory transport outage, individual Inventory/Kitchen process loss, combined process loss, recovery, and lost-response behavior. Every fault attempt retained an open review with no history, and recovery required a fresh decision. The environment was destroyed after capture.
...
The live harness collects ten scenarios and uses eight one-shot fixtures. A token-protected loopback supervisor allow-lists Inventory and Kitchen and owns their restart/cleanup; no arbitrary command or process target is accepted. Because those services use in-memory acceptance persistence, this proves Order fail-closed behavior across transport and process loss, not production state restoration or container-orchestrator recovery. Provider/accounting, production paging, persistent dependency recovery, and live-traffic rollback evidence remain open.
