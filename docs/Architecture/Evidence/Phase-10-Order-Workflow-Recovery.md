...
The acceptance hosts Payment-compatible HTTP endpoints inside the test process, invokes Payment's production recovery Application services directly after lease expiry, and publishes the resulting reconciliation contracts from the harness. It does not run the Payment service host, its hosted recovery-worker scheduler, or its transactional outbox dispatcher. Hosted Payment scheduling and outbox delivery remain the next separate acceptance slice.
