...
Run it only with an API-compatible non-production provider after following the secret-injection and three-confirmation procedure in the [deployment guide](../../Deployment/Deployment-Guide.md). The earlier direct-recovery form passed on 2026-09-15 against a temporary local HTTPS contract fixture, including all three process terminations and project cleanup. The hosted-worker/outbox form builds and parses cleanly, but its 2026-09-16 local execution could not create the generated Compose project because Docker Desktop's backend was unavailable. No hosted pass evidence or selected external-provider evidence is retained yet; both remain release gates.
