...
The read requires `reporting.activity.read`, active organization access, and the active `nexa_connect` context. Platform Directory, Restaurant, Catalog, Media, Notification, and Payment own the accepted audit route/source vocabulary. Reporting migration 4 aligns the database constraints with those Application allow lists and must precede Payment publication. Reporting consumes `*.audit.v1` with manual acknowledgement, durable inbox deduplication, and permanent-failure dead-lettering. Publication is asynchronous and conditional on each owning service's PostgreSQL mutation and outbox configuration; the feed covers only accepted events from enablement onward and is not a complete compliance record.
