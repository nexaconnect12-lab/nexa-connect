...
Customer Portal Catalog reads use `X-Nexa-Portal-Request: customer`, `X-Nexa-Organization-Id`, and `X-Nexa-Application-Code: nexa_connect` headers. The Catalog service validates the context through Platform Directory using the forwarded bearer token before reading menu data; these headers are context, not a substitute for branch or product authorization.
