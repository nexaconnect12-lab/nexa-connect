...
CREATE ROLE nexaconnect_pos_app LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION;
CREATE ROLE nexaconnect_reporting_app LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION;
ALTER ROLE nexaconnect_pos_app PASSWORD :'runtime_password';
ALTER ROLE nexaconnect_reporting_app PASSWORD :'runtime_password';
...
SELECT format('CREATE DATABASE %I OWNER nexaconnect_migration','nexa_review_it_' || :'run_id' || '_pos') \gexec
SELECT format('CREATE DATABASE %I OWNER nexaconnect_migration','nexa_review_it_' || :'run_id' || '_reporting') \gexec
SQL

for suffix_role in 'platform platform_directory_app' 'restaurant nexaconnect_restaurant_app' 'authorization nexaconnect_authorization_app' 'pos nexaconnect_pos_app' 'reporting nexaconnect_reporting_app'; do
