...
Cash-close HTTP tests cover Reporting tenant/store access and Customer BFF session/membership boundaries with test ports. `CashCloseProjectionPostgresTests` uses generated schemas in secret-injected `NEXACONNECT_POS_INTEGRATION_DB` and `NEXACONNECT_REPORTING_INTEGRATION_DB` with `NEXACONNECT_ENVIRONMENT=Testing`; it exercises close/review/late-settlement snapshots, checkpoint/outbox rollback, duplicate/stale projection, scope filtering and SQL downgrade/re-upgrade replay. It skips without configuration. No hosted cash-close RabbitMQ acceptance is implemented; see [verification and release gates](../../../docs/API/Cash-Close-Reporting.md).
