...
Payment-review operational acceptance additionally requires `NEXACONNECT_ORDER_INTEGRATION_DB`, `NEXACONNECT_REPORTING_INTEGRATION_DB`, and `NEXACONNECT_RABBITMQ_INTEGRATION_URI`; the supported script sets `NEXACONNECT_RABBITMQ_ACCEPTANCE=1` temporarily. `scripts/test-payment-review-operations.ps1` runs exactly four cases: Order clean install, Reporting migration-13 replay, repository-produced Order contracts through hosted dispatcher restart, and hosted Reporting order-audit projection/deduplication. It rejects production-looking targets, uses generated database/schema/exchange/queue boundaries, removes isolated alert containers, and retains the TRX result under `.runstate/payment-review-operations/<run-id>/`. `EvidenceLabel` is descriptive only and never validates infrastructure identity.
