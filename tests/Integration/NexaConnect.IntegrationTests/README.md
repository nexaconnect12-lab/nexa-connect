...
The component cases use isolated resources and verify atomic intent/audit/outbox commit and rollback, organization-leading reads, append-only audit, concurrent matching idempotency, persisted capture recovery after a fresh repository/worker boundary, conflicting-key rejection, and confirmed persistent publication over a new recovery connection. Payment migrations 4 and 6 own authorization and capture recovery state; the clean-install acceptance targets 0→6→5→6. `OrderMigrationRunnerAcceptanceTests` uses `NEXACONNECT_ORDER_CLEAN_INSTALL_ACCEPTANCE=1` and proves `0→2→1→2` plus downgrade refusal for pending financial uncertainty. The coordinated PostgreSQL 17/RabbitMQ 4 run passed 14 tests locally on 2026-08-26; see the retained Phase 11 evidence. Missing opt-in configuration is reported as skipped rather than passed. Production-provider process termination and established-dispatcher recovery remain release-environment evidence.
