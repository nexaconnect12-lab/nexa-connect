...
The component cases use isolated resources and verify atomic intent/audit/outbox commit and rollback, organization-leading reads, append-only audit, concurrent matching idempotency, persisted capture recovery after a fresh repository/worker boundary, conflicting-key rejection, and confirmed persistent publication. They also discard a captured provider response before the local completion transaction, reconstruct the recovery boundary, and prove status reconciliation without repeating capture. The RabbitMQ case first establishes the production transport, closes that client connection, and verifies a replacement connection publishes the next persistent event while both deliveries remain consumable. It does not kill an operating-system process or restart the broker container. Payment migrations 4 and 6 own authorization and capture recovery state; the clean-install acceptance targets 0→6→5→6. `OrderMigrationRunnerAcceptanceTests` uses `NEXACONNECT_ORDER_CLEAN_INSTALL_ACCEPTANCE=1` and proves `0→2→1→2` plus downgrade refusal for pending financial uncertainty. The coordinated PostgreSQL 17/RabbitMQ 4 run passed all 16 tests on 2026-08-28. Missing opt-in configuration is reported as skipped rather than passed. Production-provider OS process termination remains release-environment evidence.
