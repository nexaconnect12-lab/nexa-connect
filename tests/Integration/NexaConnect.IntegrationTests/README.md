...
Cash-close HTTP tests cover Reporting tenant/store access and Customer BFF session/membership boundaries with test ports. `CashCloseProjectionPostgresTests` uses generated schemas in secret-injected `NEXACONNECT_POS_INTEGRATION_DB` and `NEXACONNECT_REPORTING_INTEGRATION_DB` with `NEXACONNECT_ENVIRONMENT=Testing`; it exercises close/review/late-settlement snapshots, checkpoint/outbox rollback, duplicate/stale projection, scope filtering and SQL downgrade/re-upgrade replay. It skips without configuration. The guarded real-process cash-close RabbitMQ/replay runner is implemented and passed locally on 2026-09-24; see [verification and release gates](../../../docs/API/Cash-Close-Reporting.md).

The POS manual-tender matrix includes hosted RabbitMQ stop/queue/restart, identical replay, and invalid-message dead-letter coverage. `scripts/test-pos-order-settlement-operations.ps1` requires this, PostgreSQL cash/PromptPay projection, and the current POS migration lifecycle to execute. Its original 3/3 evidence on 2026-09-03 exercised `0→4→3→4`; current migration acceptance separately covers `0→5→4→3→5` and review-history downgrade refusal. Sanitized historical evidence is retained under `.runstate/pos-order-settlement/13827a465971402887581cd04e25a70b`.

`scripts/test-pos-paid-workflow.ps1` composes the three-case backend matrix with 17 Windows-local protected SQLite/recovery cases, including pending settlement and Cash Review recovery. It retains separate TRX files and a boolean-only evidence summary, requires all 20 current cases to pass, and fails if DPAPI is unavailable or a case is skipped. The historical six-case matrix passed 6/6 on 2026-09-03 UTC with evidence under `.runstate/pos-paid-workflow/9c322b1ace3f422a9e7e6cd37b67658b`; later historical evidence is listed in the POS client guide. It intentionally records `interactiveWpfVerified=false` and `liveOidcVerified=false`; use it as a precondition for, not a substitute for, interactive authenticated client acceptance.
...
`PosMigrationRunnerAcceptanceTests` manages only generated `nexaconnect_pos_clean_it_<guid>` databases and invokes the actual POS runner for 0→5→4→3→5. It validates checksums/history, baseline and review tables, version-owned removal/reapply, retained cash/replay rows, real repositories, and migration-5 downgrade refusal after a committed review. Migration-4 broker recovery/replay and dead-letter routing passed in the 2026-09-03 UTC guarded matrix; broader release-environment validation remains deployment-owned.
...
Authorization permission-backfill acceptance uses `NEXACONNECT_AUTHORIZATION_CLEAN_INSTALL_ACCEPTANCE=1` and the same disposable PostgreSQL administrator in a safe environment. `AuthorizationMigrationRunnerAcceptanceTests` manages only `nexaconnect_authorization_clean_it_<guid>`, seeds existing tenant-admin, store-manager, and accountant roles, verifies the current Kitchen, Order review, manual-tender, and migration-7 POS Cash Review grants including accountant read-only separation, then proves removal/reapplication.

Payment-review operational acceptance additionally requires `NEXACONNECT_ORDER_INTEGRATION_DB`, `NEXACONNECT_REPORTING_INTEGRATION_DB`, and `NEXACONNECT_RABBITMQ_INTEGRATION_URI`; the supported script sets `NEXACONNECT_RABBITMQ_ACCEPTANCE=1` temporarily. `scripts/test-payment-review-operations.ps1` runs exactly eight cases: four HTTP boundary/attribution tests using controlled auth/repository doubles, plus Order clean install, Reporting migration-13 replay, repository-produced Order contracts through hosted dispatcher restart, and hosted Reporting order-audit projection/deduplication. It rejects production-looking targets, uses generated database/schema/exchange/queue boundaries, removes isolated alert containers, and retains the TRX result under `.runstate/payment-review-operations/<run-id>/`. `EvidenceLabel` is descriptive only and never validates infrastructure identity.
...
Run `scripts/test-cash-close-recovery.ps1 -ConfirmDisposableInfrastructure` for the generated PostgreSQL/RabbitMQ matrix. The partial `CashCloseProjectionPostgresTests` class now includes a real child-process pipeline case alongside its repository case; the runner requires exactly two passing cases and no skips, removes its project volumes and emits bounded evidence after cleanup. It exercises committed-before-ack consumer termination, broker outage/restart, concurrent source publication, duplicate/stale/poison delivery and Reporting rebuild via the CLI twice. Local Windows acceptance passed 2/2 with zero skips on 2026-09-24, including restricted credentials and unchanged source fingerprints; see [coverage and evidence boundaries](../../../docs/Deployment/Cash-Close-Recovery.md).
