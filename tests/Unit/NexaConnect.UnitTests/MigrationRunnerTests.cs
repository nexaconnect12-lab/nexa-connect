...
public sealed class MigrationRunnerTests
{
    ...
    public async Task Payment_product_integration_preserves_the_baseline_outbox_on_rollback()
    {
        ...
        IReadOnlyList<MigrationStep> rollback = catalog.CreatePlan(applied, 1);
        Assert.Equal([5, 4, 3, 2], rollback.Select(step => step.Migration.Version).ToArray());
        Assert.All(rollback, step => Assert.Equal(MigrationDirection.Down, step.Direction));
        Assert.Contains("provider_capture_id", rollback[0].Migration.DownSql, StringComparison.Ordinal);
        Assert.Contains("provider_authorization_id", rollback[1].Migration.DownSql, StringComparison.Ordinal);
        Assert.Contains("DROP TABLE payment_audit_records", rollback[3].Migration.DownSql, StringComparison.Ordinal);
        Assert.Contains("DROP COLUMN organization_id", rollback[3].Migration.DownSql, StringComparison.Ordinal);
        Assert.Contains("HAVING count(*)>1", rollback[3].Migration.DownSql, StringComparison.Ordinal);
