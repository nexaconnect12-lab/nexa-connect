...
Payment Review acceptance has a generated PostgreSQL/RabbitMQ/alert operations project whose 13-case matrix passed locally, plus a joined PostgreSQL/Keycloak/RabbitMQ/Toxiproxy project. The joined launcher provisions accounts and five fixtures, starts seven loopback application hosts, routes Order-to-Inventory through the isolated proxy, requires 7/7 real-OIDC browser scenarios, and verifies process/Compose cleanup. The joined run passed locally on 2026-09-02. Inventory process loss, Kitchen/combined outage rehearsal, provider evidence, production paging, and live-traffic rollback remain open. See `docs/Deployment/Payment-Review-Release-Checklist.md`.

`NexaConnect.PaymentReviewAcceptance` provisions joined-browser data through Platform Directory, Restaurant, Authorization, and Order-owned Application/Infrastructure boundaries. The joined launcher owns a generated PostgreSQL/Keycloak/RabbitMQ/Toxiproxy project, applies four migrations, creates disposable reader/resolver users, provisions marked fixtures, starts seven loopback application hosts, wires a run-scoped Order-to-Inventory proxy, requires the seven-scenario Playwright suite, and cleans up exact processes and Compose resources. Exact run-scoped database names and bounded service probes reject detected fixture state before writes. Local 7/7 joined evidence passed on 2026-09-02; production provider, paging, multi-dependency, and rollback rehearsals remain gates.
