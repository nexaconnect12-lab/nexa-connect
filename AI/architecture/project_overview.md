...
The current Customer Portal BFF is `NexaConnect.CustomerBff`. It keeps tokens server-side and protects the selected tenant in an HTTP-only cookie. Phase 8 now includes Platform Directory-owned customer membership list/change behavior with exact-organization access, customer owner/admin roles, self-mutation protection, optimistic concurrency, and transactional audit records. Remaining operational UI is staged.
