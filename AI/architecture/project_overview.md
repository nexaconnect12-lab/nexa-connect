...
The WPF POS separates Checkout, Payment, Shift & cash, and Terminal & sync, with touch product tiles, filtering, explicit totals, and terminal-bound SQLite recovery/outbox state. Shift & cash reads a PostgreSQL-authoritative, cashier/terminal-scoped reconciliation with opening cash, signed movements, expected cash, counted variance, history, and concurrency version. Close compares that reviewed version and scope in the update that stores expected/count/variance and advances the version. Terminal & sync lists unresolved operation metadata without protected payloads. The guarded joined SQLite-backed local OIDC/WPF cash acceptance passed on 2026-09-10; server-side intermediate-workflow recovery and reconciliation history/supervisor workflows remain open. See the [POS cashier acceptance guide](../../docs/Deployment/POS-Cashier-Acceptance.md).
