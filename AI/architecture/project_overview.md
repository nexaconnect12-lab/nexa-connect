...
Current implementation status: the repository provides solution scaffolding, JWT validation, local identity/infrastructure configuration, schema-first migrations, sample-data tooling, PostgreSQL-backed Platform Directory and POS slices, PostgreSQL Customer and Payment adapters, a durable PostgreSQL/RabbitMQ outbox and PostgreSQL aggregate/idempotency repository for Order, a public place-order workflow endpoint, a web BFF session flow, a WPF POS PKCE sign-in and shift open/close client slice, executable Catalog, Inventory, Order, Payment, Customer, and Notification API slices, and an Order workflow that coordinates Catalog/Menu, Inventory, Kitchen, and Payment through bounded-context ports and versioned integration events. Catalog, Inventory, Notification durable repositories, production provider credentials, and deployment hardening remain planned.
...
The Restaurant authorization-scope controller and the Platform Directory and Authorization persistence paths now follow these boundaries: Application owns the ports and Infrastructure owns the PostgreSQL adapters. The POS shift flow likewise uses an Application service and Infrastructure persistence adapter; the newly added cash-session and terminal-enrollment endpoints still have controller-level orchestration and are tracked as the next POS layering refactor. New work must not copy the remaining legacy patterns, and material changes to that code must move it toward this structure.
