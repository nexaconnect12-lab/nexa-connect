...
The single-store cash-close report is implemented through the Customer Portal/BFF. POS migration 6 adds an opt-in scanner that locks closed sessions and atomically publishes current versioned snapshots plus checkpoints through its outbox; intermediate changes can coalesce. Reporting migration 15 stores newest facts and hashed receipts, rejecting conflicting delivery and ignoring stale versions. Every read revalidates exact-store `pos.cash-review.read` through POS. Bounded UTC pages show capture/projection times without totals or completeness claims. POS migration 7 adds append-only replay attribution; a manifest-pinned retained-event CLI, real-process recovery runner and backlog/retry alerts are implemented. Live database/broker execution and alert delivery remain open. See [recovery runbook](../../docs/Deployment/Cash-Close-Recovery.md). See [contract](../../docs/API/Cash-Close-Reporting.md) and [ADR-012](../../docs/Architecture/Decisions/ADR-012-cash-close-snapshot-reporting.md).
