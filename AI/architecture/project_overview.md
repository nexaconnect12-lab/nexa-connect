...
Some current service code predates these mandatory boundaries, including direct database access in the Restaurant API controller and persistence stores that are not yet separated behind Application interfaces. The POS shift flow now uses an Application service and Infrastructure persistence adapter; the newly added cash-session and terminal-enrollment endpoints still have controller-level orchestration and are tracked as the next POS layering refactor. New work must not copy the remaining legacy patterns, and material changes to that code must move it toward this structure.
