...
POS cash-movement replay has successful live PostgreSQL 17 evidence for exact/concurrent idempotency, rollback, terminal/subject denial, active scope, and shift concurrency. POS migration 4 adds the immutable manual-tender projection. The upgraded protected SQLite/PostgreSQL/RabbitMQ/migration runner passed 17/17, including current-user recovery and `0→4→3→4`. Terminal-bound SQLite persistence is implemented and has restart, migration, corruption, scope, queue-transition, and joined local cash-checkout evidence. Physical-terminal acceptance, server-side intermediate-workflow recovery, and broader order/shift/device synchronization remain open.
