...
POS manual-tender hardening includes hosted RabbitMQ recovery/dead-letter acceptance, a guarded PowerShell evidence runner, correlated telemetry, and Prometheus backlog/dead-letter/retry rules. Disposable environment execution remains a release gate.

The Bangkok manual-tender path uses Order 5, Authorization 6, Reporting 14, and POS 4. The optional POS consumer validates the event-time shift/session and projects each Order once. Cash posts to that THB session and can reconcile it after close; PromptPay never changes drawer totals. Cashier UI and execution of the checked-in live acceptance matrix remain next.
