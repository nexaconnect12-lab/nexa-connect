...
Payment intent creation retains domain validation, organization-leading persistence, conflict-safe idempotency, append-only audit, and transactional publication. Authorization, capture, and pre-capture void recovery distinguish definitive outcomes from uncertainty. Payment migrations 6-7 own capture/void recovery and Order migrations 2-3 durably consume their outcomes. Order completes only confirmed capture, compensates definitive failure or confirmed void through downstream idempotency keys, retains work during uncertainty, and validates persisted organization plus payment-intent ownership. Reporting migrations 11-12 accept the audit vocabulary. Local migration and recovery evidence passed. Provider selection is explicit: `Disabled` is fail-closed and makes no network calls or recovery claims, while `GenericHttp` enables the provider boundary after configuration. The Order domain now defines the Bangkok manual-tender invariants for cash and explicitly receipt-confirmed PromptPay: exact THB amount, matching tenant/branch, bounded bank reference, terminal/operator/idempotency identity, eligible state, and exclusion of provider-bound Orders. No operational endpoint or persistence exists yet. A guarded provider-neutral runner can validate a real sandbox TLS handshake, configured-credential acceptance, capture/void replay identity, and status reconciliation with distinct disposable authorization references; it has not yet run against a selected provider. Manual-tender persistence/API/POS delivery, invalid-credential rejection, live RabbitMQ void replay, provider-specific rate-limit/lost-response behavior, production paging/escalation, and live-traffic rollback remain environment-specific gates. Refunds and settlement remain planned.
