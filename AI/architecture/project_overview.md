...
Payment intent creation retains domain validation, organization-leading persistence, conflict-safe idempotency, append-only audit, and transactional publication. The development authorization slice now adds an exact Order-workload endpoint, a concurrency-controlled `authorizing` lease, provider I/O outside database transactions, sanitized terminal outcomes, and transactional authorization/audit events through Payment migration 3 and Reporting migration 8. Order accepts only `authorized`, not `pending`. Default automated coverage passes; opt-in PostgreSQL authorization, 0→3→1→3 migration, Reporting replay, and provider-environment evidence remain release gates. Abandoned-lease recovery, authoritative legacy mapping, capture, void, refunds, and reconciliation remain planned.
