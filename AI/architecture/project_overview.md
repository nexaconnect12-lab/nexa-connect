...
The WPF POS now separates Checkout, Payment, Shift & cash, and Terminal & sync, with touch product tiles, name/station filtering, quantity controls, and explicit currency totals. A terminal-bound, schema-versioned SQLite database stores DPAPI-protected operational/outbox payloads with WAL/FULL durability, explicit retry states, interrupted-send recovery, completed acknowledgements, and fail-closed legacy migration. The client routes tenant-scoped menu reads directly to Catalog and retains the original placement command for identical restart/retry. A guarded verifier now joins an explicitly confirmed interactive run to amount-matched Order/POS cash records, closed session/shift state, cleared credentials, and an integrity-checked empty SQLite operational queue. The pre-upgrade local OIDC/WPF cash checkout passed on 2026-09-10; upgraded SQLite live evidence and server-side intermediate-workflow recovery remain open. See the [POS cashier acceptance guide](../../docs/Deployment/POS-Cashier-Acceptance.md).
