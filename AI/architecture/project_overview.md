...
Payment intent creation retains domain validation, organization-leading persistence, conflict-safe idempotency, append-only audit, and transactional publication. Authorization recovery distinguishes and reconciles uncertain outcomes. The immediate-capture development slice adds a separate Order-only provider call after authorization, `capturing`/`captured`/`capture_unknown` state, sanitized capture references, and transactional capture/audit publication through Payment migration 5 and Reporting migration 10. Order becomes paid only after capture and retains work for uncertainty. Default automated coverage passes; live PostgreSQL, 0→5→1→5 migration, Reporting replay, provider, and capture-recovery evidence remain release gates. Void, refunds, and settlement remain planned.

Recorded next implementation (planning only): Payment capture recovery with provider status lookup, recoverable capture leases, bounded background reconciliation, conservative Order completion/compensation, transactional `PaymentCaptureReconciledV1`, durable Order consumption, Payment migration 6, Reporting migration 11, and live `0→6→5→6` plus provider/recovery evidence. It precedes void/reversal, refunds, compensation-policy expansion, and end-of-day settlement.
