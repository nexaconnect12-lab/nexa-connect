...
The initial Phase 7 Product Owner Portal compatibility implementation lives in `src/Frontend/apps/product-owner-portal`; durable ownership remains with the future shared-platform repository under ADR-006. Its current screens cover creation/status and read slices for the control plane, audit, support requests, and Platform Directory summary counts. Organization/user lifecycle completion and support approval/revocation remain planned. Product administration remains a separate boundary reached only through controlled links.

The current Customer Portal BFF is `NexaConnect.CustomerBff`. It keeps the authentication ticket and OIDC tokens in a server-side distributed ticket store, refreshes expiring access tokens or clears an unusable session, calls Platform Directory for current organization/product access, and stores only an encrypted HTTP-only tenant selection cookie in the browser; product APIs retain final authorization responsibility.
