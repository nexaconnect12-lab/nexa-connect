...
The Bangkok manual-tender backend now uses Order migration 5, Authorization migration 6, and Reporting migration 14. The authorized Order endpoint records exact-total THB cash or receipt-confirmed PromptPay, excludes provider-bound Orders, and atomically commits Paid state, an immutable settlement, a safe integration event, and audit outbox. Tenant/branch/idempotency uniqueness plus a request fingerprint make matching replay stable and conflicting reuse deterministic. POS consumption and cashier UI delivery remain next.
...
Payment authorization, capture, and pre-capture void recovery distinguish definitive outcomes from uncertainty and default to the fail-closed `Disabled` provider. Order migration 5 operationalizes Bangkok cash and explicitly receipt-confirmed PromptPay through an authorized endpoint and immutable, atomic persistence. Authorization migration 6 and Reporting migration 14 own its permission and audit vocabulary. The safe settlement event is ready for future idempotent POS cash-session consumption. POS UI/consumption, provider-specific live evidence, refunds, and settlement remain planned.
