...
Inventory Phase 10 publication is implemented for tenant-scoped stock set, reservation create, and release: those PostgreSQL mutations commit versioned integration events and safe audit state through the migration-1 outbox and migration-5 append-only audit boundary. Trusted unscoped PostgreSQL paths remain legacy/non-publishing. Active-order retries do not decrement stock twice. Live database concurrency/migration and RabbitMQ recovery evidence remain Phase 11 exit work.
