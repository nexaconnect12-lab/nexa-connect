...
Current implementation status: the repository provides solution scaffolding, JWT validation, local identity/infrastructure configuration, schema-first migrations, sample-data tooling, PostgreSQL-backed Platform Directory and POS slices, a web BFF session flow, a WPF POS PKCE sign-in and shift open/close client slice, executable Catalog, Inventory, Order, Payment, Customer, and Notification API slices, and an Order workflow that coordinates Catalog/Menu, Inventory, Kitchen, and Payment through bounded-context ports and versioned integration events. The six newly replaced service slices currently use explicit in-memory Infrastructure adapters; production PostgreSQL/provider adapters, RabbitMQ delivery, durable workflow persistence, durable offline synchronization, and broader product resource-level authorization remain planned.
