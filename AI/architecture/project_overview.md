...
POS cash-movement replay has successful live PostgreSQL 17 evidence for exact/concurrent idempotency, rollback, terminal/subject denial, active scope, and shift concurrency. POS migration 4 owns the immutable manual-tender projection; migration 5 owns review state/history and refuses downgrade after review data exists. The migration lifecycle now exercises `0→5→4→3→5` and the post-review guard. Terminal-bound SQLite recovery, version-fenced cashier reconciliation, and online supervisor review have focused Application, HTTP, client, migration, and live PostgreSQL coverage. Fresh interactive supervisor/physical-terminal acceptance, multi-store review, exports/Reporting projection, server-side intermediate-workflow recovery, and broader synchronization remain open.
