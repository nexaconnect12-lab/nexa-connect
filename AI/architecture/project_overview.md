...
The first authenticated product adapters forward Customer Portal requests to Catalog, Inventory, and the Order place workflow with the server-held bearer token and validated tenant context. The BFF derives organization and branch identifiers from its protected tenant selection and route. Catalog verifies organization access through Platform Directory and validates that the selected branch belongs to that organization by calling Restaurant with the Catalog workload identity; Order repeats those checks with its own workload identity before executing the workflow. Product services remain responsible for final resource authorization.
