...
Payment intent creation retains domain validation, organization-leading persistence, conflict-safe idempotency, append-only audit, and transactional publication. Authorization and capture recovery distinguish definitive outcomes from uncertainty. Capture-specific leases, bounded provider status lookup, reconciliation timestamps, and transactional `PaymentCaptureReconciledV1` publication are implemented through Payment migration 6. Order migration 2 durably consumes reconciliation: authorization alone never completes an order, only captured funds mark it paid, definitive failure performs idempotent Inventory/Kitchen compensation, and unresolved or exhausted recovery remains payment-pending. Reporting migration 11 accepts the audit vocabulary. Sixteen coordinated cases passed locally on 2026-08-28. Safety-gated harnesses also passed child-process recovery, full RabbitMQ-container restart, isolated firing/resolved webhook delivery, and TRX-confirmed Payment/Order rollback-forward lifecycles. Concrete-provider, production paging/escalation, and live-traffic rollback validation remain environment-specific gates. Void, refunds, and settlement remain planned.

Payment capture recovery is implemented with provider status lookup, recoverable capture leases, bounded background reconciliation, conservative Order completion/compensation, transactional `PaymentCaptureReconciledV1`, durable Order consumption, Payment migration 6, Order migration 2, and Reporting migration 11. Operators can pause only the capture-recovery worker through `PaymentProvider:CaptureRecoveryEnabled`; safety-gated scripts repeat the disposable matrix, execute child-process/broker faults, route synthetic firing/resolved notifications through an isolated Alertmanager webhook, and verify generated-database rollback/forward recovery. Payment and Order publish safe backlog/age gauges; Prometheus scrapes RabbitMQ queue metrics and evaluates checked-in development alerts. Concrete-provider acceptance, production paging/escalation, and live-traffic drain/recovery remain release gates. Void/reversal accepts only trusted Order calls for authorized, uncaptured intents, uses a stable provider idempotency key, separates failed from uncertain outcomes, and rejects captured intents. Order void consumption, refunds, compensation-policy expansion, and end-of-day settlement remain planned.

Payment migration 7 now makes void/reversal durable with PostgreSQL leases, bounded status reconciliation, sanitized provider references, transactional lifecycle/audit outbox publication, and guarded downgrade. Reporting migration 12 accepts the vocabulary. Local PostgreSQL tests passed successful transactional unknown-to-voided recovery and `0→7→6→7`. Order void-event consumption, dedicated telemetry/alerts, concrete-provider evidence, refunds, and settlement remain open.

Kitchen ticket lifecycle now enforces queued/in-progress/ready/completed/cancelled invariants, exact Order workload creation/compensation, dedicated Kitchen workload authorization-scope lookup, migration-backed operator permissions, tenant-authorized operator reads/transitions, conflict-safe station snapshots, organization-leading PostgreSQL, append-only history/audit, and transactional Kitchen lifecycle/audit events. Reporting migration 5 provides compatible projection and replay. Five coordinated acceptances passed against PostgreSQL 17 and RabbitMQ, including multi-station creation and the actual 0→3→2→3 runner lifecycle. The ticket-level Phase 10 slice is closed; item/KDS/offline workflows remain planned.
...
POS cash-movement replay has successful live PostgreSQL 17 evidence: seven isolated-schema cases cover exact/concurrent idempotency, atomic rollback, terminal/shift-subject denial, active terminal scope, shift persistence/concurrency, and duplicate-open conflict. The live run found and corrected an Npgsql multi-command completion issue. The actual migration runner also passed 0→3→2→3 in a validated disposable database, proving checksums/history, schema ownership and downgrade retention, and repository writes around re-upgrade. Production SQLite client persistence and broader order/shift/device synchronization remain open.
...
The recommended restaurant topology uses an always-on branch edge service so POS terminals, self-service kiosks, and kitchen displays can coordinate over the local network during WAN or cloud outages. The branch-edge decision must be confirmed before implementation. Windows POS and kiosk clients should also use local SQLite outboxes for brief device-to-edge outages. The current WPF cash-replay scaffold persists a JSON queue before its first HTTP attempt; it is not the production SQLite or atomic local business-state/outbox implementation.
