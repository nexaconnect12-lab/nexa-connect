...
Payment intent creation retains domain validation, organization-leading persistence, conflict-safe idempotency, append-only audit, and transactional publication. Authorization now distinguishes authorized, declined, and unknown provider outcomes, stores recoverable lease ownership/expiry and bounded attempts, and reconciles uncertain intents through a stable Payment intent lookup. Payment migrations 3-4 and Reporting migrations 8-9 publish sanitized lifecycle/reconciliation events. Order retains Inventory/Kitchen work in `payment_pending` for uncertain outcomes and later reconciles to paid or compensated. Default automated coverage passes; opt-in PostgreSQL authorization, 0→4→3→4 migration, Reporting replay, and provider-environment evidence remain release gates. Capture, void, refunds, and financial settlement remain planned.
