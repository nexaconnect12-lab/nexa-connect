...
The single-store cash-close report is implemented through the Customer Portal/BFF. POS migration 6 adds an opt-in scanner that locks closed sessions and atomically publishes current versioned snapshots plus checkpoints through its outbox; intermediate changes can coalesce. Reporting migration 15 stores newest facts and hashed receipts, rejecting conflicting delivery and ignoring stale versions. Every read revalidates exact-store `pos.cash-review.read` through POS. Bounded UTC pages show capture/projection times without totals or completeness claims. Live database/broker acceptance and replay tooling remain open. See [contract](../../docs/API/Cash-Close-Reporting.md) and [ADR-012](../../docs/Architecture/Decisions/ADR-012-cash-close-snapshot-reporting.md).
