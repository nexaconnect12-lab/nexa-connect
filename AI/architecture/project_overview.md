...
Payment intent creation retains domain validation, organization-leading persistence, conflict-safe idempotency, append-only audit, and transactional publication. Authorization and capture recovery distinguish definitive outcomes from uncertainty. Capture-specific leases, bounded provider status lookup, reconciliation timestamps, and transactional `PaymentCaptureReconciledV1` publication are implemented through Payment migration 6. Order migration 2 durably consumes reconciliation: authorization alone never completes an order, only captured funds mark it paid, definitive failure performs idempotent Inventory/Kitchen compensation, and unresolved or exhausted recovery remains payment-pending. Reporting migration 11 accepts the audit vocabulary. Sixteen coordinated PostgreSQL 17/RabbitMQ 4 cases passed locally on 2026-08-28. A separately gated harness passed actual child-process termination after an HTTP capture response, fresh-process reconciliation, and an established publisher surviving a full RabbitMQ Compose-container restart. The provider endpoint is a local fixture; concrete-provider and alert-delivery validation remain environment-specific gates. Void, refunds, and settlement remain planned.

Payment capture recovery is implemented with provider status lookup, recoverable capture leases, bounded background reconciliation, conservative Order completion/compensation, transactional `PaymentCaptureReconciledV1`, durable Order consumption, Payment migration 6, Order migration 2, and Reporting migration 11. Operators can pause only the capture-recovery worker through `PaymentProvider:CaptureRecoveryEnabled`; a safety-gated release script repeats the disposable matrix, and a second harness executes exact child-process termination plus verified RabbitMQ-container restart. Payment and Order publish safe backlog/age gauges; Prometheus scrapes RabbitMQ queue metrics and evaluates checked-in development alerts. Local process-kill and full-broker-restart evidence passed with a local HTTP provider fixture. Concrete-provider acceptance and production paging rehearsal remain release gates. It precedes void/reversal, refunds, compensation-policy expansion, and end-of-day settlement.
