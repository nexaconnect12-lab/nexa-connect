...
Payment Review acceptance now has a generated Compose project with ephemeral PostgreSQL/RabbitMQ/alert services, loopback dynamic ports, in-memory credentials, scoped cleanup and linked sanitized evidence. The expanded 13-case matrix passed locally on 2026-08-31. A separate six-scenario real-OIDC browser harness rejects missing settings and requires a dedicated disposable realm/fixtures; joined execution and real downstream-outage rehearsal remain open. It does not provision application services or accounts. See `docs/Deployment/Payment-Review-Release-Checklist.md`.
