...
POS cash-movement replay has successful live PostgreSQL 17 evidence: seven isolated-schema cases cover exact/concurrent idempotency, atomic rollback, terminal/shift-subject denial, active terminal scope, shift persistence/concurrency, and duplicate-open conflict. The live run found and corrected an Npgsql multi-command completion issue. Migration-runner 0→3→2→3 evidence, production SQLite client persistence, and broader order/shift/device synchronization remain open.
