...
Payment Review acceptance now has a generated Compose project with ephemeral PostgreSQL/RabbitMQ/alert services, loopback dynamic ports, in-memory credentials, scoped cleanup and linked sanitized evidence. The expanded 13-case matrix passed locally on 2026-08-31. A separate seven-scenario real-OIDC browser harness rejects missing settings and requires a dedicated disposable realm, five fixtures, and a run-scoped Inventory fault proxy. It verifies fail-closed behavior at the configured Order-to-Inventory transport boundary and explicit recovery without automatic decision replay; joined execution, Inventory process loss, Kitchen/combined outage rehearsal, and provider evidence remain open. It does not provision application services or accounts. See `docs/Deployment/Payment-Review-Release-Checklist.md`.

`NexaConnect.PaymentReviewAcceptance` provisions joined-browser data through Platform Directory, Restaurant, Authorization, and Order-owned Application/Infrastructure boundaries. It accepts only exact run-scoped loopback database names, concurrently rejects detected fixture root/state data in all four databases before writing, and emits identifier-only JSON for the browser harness. Operators still verify the complete databases are fresh. Realm/database creation, application/proxy launch, and cleanup remain orchestration work.
...
Payment intent creation retains domain validation, organization-leading persistence, conflict-safe idempotency, append-only audit, and transactional publication. Authorization, capture, and pre-capture void recovery distinguish definitive outcomes from uncertainty. Payment migrations 6-7 own capture/void recovery and Order migrations 2-3 durably consume their outcomes. Order completes only confirmed capture, compensates definitive failure or confirmed void through downstream idempotency keys, retains work during uncertainty, and validates persisted organization plus payment-intent ownership. Reporting migrations 11-12 accept the audit vocabulary. Local migration and recovery evidence passed; live RabbitMQ void replay, concrete-provider behavior, production paging/escalation, and live-traffic rollback remain environment-specific gates. Refunds and settlement remain planned.
