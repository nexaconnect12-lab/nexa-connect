...
The current Customer Portal BFF is `NexaConnect.CustomerBff`. It keeps tokens server-side and protects the selected tenant in an HTTP-only cookie; product APIs retain final authorization responsibility. The Phase 8 React context/navigation shell lives in `src/Frontend/apps/customer-portal`. Pages are client-gated by current access, while `/tenant` and `/features/*` revalidate the exact pair server-side. Operational dashboards and product-owned feature UI remain staged.
